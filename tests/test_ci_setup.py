...
def test_github_workflows_exist():
    ...
    assert (workflows_dir / "sync-to-downstream.yml").exists(), "sync-to-downstream.yml should exist"
    assert (workflows_dir / "sync-from-template.yml").exists(), "sync-from-template.yml should exist"
    assert (workflows_dir / "template-cleanup.yml").exists(), "template-cleanup.yml should exist"
...
def test_sync_scripts_exist():
    ...
    if os.name != 'nt':  # Not Windows
        ...
        assert os.access(link_script, os.X_OK), "link-existing-repo.sh should be executable"
...
def test_preserve_patterns():
    ...
    if sync_script.exists():
        ...
        assert "PRESERVE_PATTERNS" in content, "Should have preservation patterns defined"
