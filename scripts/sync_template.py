    ...
    def find_repos_with_template(self) -> List[str]:
        ...
        try:
            ...
            cmd = [
                'gh', 'api', '--paginate',
                f'/orgs/{org}/repos',
                '--jq', '.[] | select(.topics[]? | contains("kailash-template")) | .full_name'
            ]
    ...
    def sync_repo(self, downstream_repo: str) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                branch_name = f"template-sync-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        ...
        for pattern in SYNC_PATTERNS:
            for file_path in template_path.glob(pattern):
                if file_path.is_file():
                    ...
                    preserve_existing = any(str(relative_path).startswith(preserve_pattern.rstrip('/')) 
                    ...
                    in_merge_dir = any(str(relative_path).startswith(merge_pattern.rstrip('/'))
                                     for merge_pattern in MERGE_DIRECTORIES)
                    
                    if in_merge_dir and dest_path.exists():
                        logger.info(f"Preserving existing file in merge directory: {relative_path}")
                        continue
                    
                    # Check if file needs special merge handling
                    if str(relative_path) in MERGE_FILES:
                        if self.merge_file(file_path, dest_path, MERGE_FILES[str(relative_path)]):
                            changes_made = True
                    else:
                        # Simple copy
...
# No need to check for sync workflow - entire .github folder is synced
    ...
    def merge_pyproject(self, src: Path, dst: Path) -> bool:
        ...
        try:
            ...
            if 'project' in downstream_data and 'dependencies' in downstream_data['project']:
                project_deps = set(downstream_data['project']['dependencies'])
                template_deps = set(template_data.get('project', {}).get('dependencies', []))
    ...
    def create_pr(self, repo: str, branch: str):
        ...
        try:
            subprocess.run([
                ...
                '--head', branch,
                '--base', 'main',
                '--title', f'Sync template updates from {self.template_repo}',
                '--body', f"""## Template Sync
...
def main():
    ...
    if not template_repo or not github_token:
        logger.error("Missing required environment variables: GITHUB_REPOSITORY, GITHUB_TOKEN")
        sys.exit(1)
    ...
    logger.info(f"Successfully synced {success_count}/{len(downstream_repos)} repositories")
