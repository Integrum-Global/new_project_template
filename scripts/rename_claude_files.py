...
def rename_claude_file(repo: str) -> bool:
    ...
    if not has_claude and has_claude_upper:
        print(f"  ✅ Already has CLAUDE.md (uppercase)")
        return False
    
    if not has_claude and not has_claude_upper:
        print(f"  ❌ No Claude.md or CLAUDE.md found")
        return False
    
    if has_claude and has_claude_upper:
        print(f"  ⚠️  Both Claude.md and CLAUDE.md exist - needs manual review")
        return False
    
    # Get file content and SHA
    print(f"  📥 Fetching Claude.md content...")
    ...
    cmd = ["gh", "api", f"/repos/{repo}/git/refs/heads/{default_branch}", "--jq", ".object.sha"]
    base_sha = run_command(cmd).strip()
    
    if not base_sha:
        print(f"  ❌ Failed to get base commit SHA")
        return False
    ...
    if result.returncode != 0:
        print(f"  🗑️  Deleting existing branch...")
        cmd = ["gh", "api", f"/repos/{repo}/git/refs/heads/{branch_name}", "-X", "DELETE"]
        run_command(cmd)
    ...
    cmd = [
        ...
        "-X", "PUT",
        "-f", f"message=rename: Claude.md to CLAUDE.md for consistency",
        "-f", f"content={content}",
        "-f", f"branch={branch_name}"
    ]
    result = subprocess.run(cmd, capture_output=True)
    if result.returncode != 0:
        print(f"  ❌ Failed to create CLAUDE.md")
        return False
    
    # Delete Claude.md
    print(f"  🗑️  Deleting Claude.md...")
    cmd = [
        "gh", "api", f"/repos/{repo}/contents/Claude.md",
        "-X", "DELETE",
        "-f", f"message=rename: remove old Claude.md (now CLAUDE.md)",
        "-f", f"sha={sha}",
        "-f", f"branch={branch_name}"
    ]
    result = subprocess.run(cmd, capture_output=True)
    if result.returncode != 0:
        print(f"  ❌ Failed to delete Claude.md")
        return False
    ...
    cmd = [
        ...
        "--head", branch_name,
        "--base", default_branch,
        "--title", "rename: Claude.md to CLAUDE.md for consistency",
        "--body", """## Rename Claude.md to CLAUDE.md
