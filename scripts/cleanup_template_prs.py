...
def get_template_sync_prs(repo: str) -> List[Dict]:
    ...
    cmd = [
        ...
        "--jq", '.[] | select(.title | contains("Sync template")) | {number: .number, title: .title, created_at: .created_at, head_ref: .head.ref}'
    ]
    ...
    for i, pr in enumerate(prs):
        ...
        print(f"  {status} #{pr['number']}: {pr['title']} (created: {pr['created_at']})")
...
def close_pr(repo: str, pr_number: int, branch: str) -> bool:
    ...
    close_cmd = [
        ...
        "--repo", repo,
        "--comment", "Closing older template sync PR. Keeping only the latest sync."
    ]
