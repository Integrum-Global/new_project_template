...
def check_claude_needs_fix(repo: str) -> bool:
    ...
    print("  📋 Checking CLAUDE.md content...")

    # Get CLAUDE.md content
    returncode, stdout, _ = run_command(
        ["gh", "api", f"/repos/{repo}/contents/CLAUDE.md", "--jq", ".content"],
        check=False,
    )
    ...
    if has_new_paths:
        print("  ✅ CLAUDE.md already has correct paths")
        return False
    elif has_old_paths:
        print("  ⚠️  CLAUDE.md needs path updates")
        return True
    else:
        print("  ❓ CLAUDE.md structure unclear, checking...")
...
def create_claude_fix_pr(repo: str) -> bool:
    ...
    returncode, stdout, _ = run_command(
        [
            ...
            "title,number",
            "--jq",
            '.[] | select(.title | contains("CLAUDE.md")) | .number',
        ],
        check=False,
    )
...
def main():
    ...
    if success > 0:
        print("\n🎯 Next steps:")
        print("  1. Review and merge the created PRs")
        print("  2. All downstream repos will have correct CLAUDE.md paths")
