...
class TestBlogApplicationE2E:
    ...
    async def blog_dataflow(self):
        ...
        config = DataFlowConfig(
            database_url="postgresql://test:test@localhost:5434/test_blog_app",
...
Modern applications require thoughtful database design. Here are the patterns we use...
        ...
        commenting_workflow.add_node(
            ...
            "author_reply",
            {
                ...
                "author_id": author["id"],
                "post_id": published_post_1["id"],
                "parent_comment_id": None,  # Will be set via connection
                "is_approved": True,
            },
        )
        ...
        commenting_workflow.add_connection(
            "create_comment_0", "author_reply", "id", "parent_comment_id"
        )
        ...
        discovery_workflow.add_node(
            "BlogPostReadNode", "get_post_details", {"id": published_post_1["id"]}
        )
        ...
        assert len(published_posts) == 2  # Two published posts
        assert len(tech_posts) >= 1  # At least the DataFlow post
        assert len(featured_posts) == 1  # One featured post
        ...
        user_stats = analytics_results["get_user_stats"]
        content_stats = analytics_results["get_content_stats"]

        assert user_stats["total_users"] == 2
        assert user_stats["active_users"] == 2
        assert content_stats["total_posts"] == 3
        assert content_stats["published_posts"] == 2
        assert content_stats["draft_posts"] == 1
        ...
        print(f"✅ Published Posts: {content_stats['published_posts']}")
        ...
        return {
            "users_created": user_stats["total_users"],
            "posts_published": content_stats["published_posts"],
