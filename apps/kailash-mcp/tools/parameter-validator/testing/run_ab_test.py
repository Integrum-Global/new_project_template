...
class MCPToolABTest:
    ...
    def setup_test_environment(self, num_challenges: int = 20) -> Dict[str, Any]:
        ...
        print(f"🔄 Conditions: baseline ({len([s for s in test_sessions if s['condition'] == 'baseline'])}), "
...
"""
    ...
    def evaluate_test_session(self, 
                            session_id: str,
                            workflow_code: str, 
                            development_time: float,
                            debug_iterations: int = 0) -> Dict[str, Any]:
        """
        ...
        if not test_plan_file.exists():
            raise FileNotFoundError("Test plan not found. Run setup_test_environment() first.")
        
        with open(test_plan_file, 'r', encoding='utf-8') as f:
            test_plan = json.load(f)
        
        # Find session details
        session = next((s for s in test_plan["test_sessions"] if s["session_id"] == session_id), None)
        if not session:
            raise ValueError(f"Session {session_id} not found in test plan")
        
        # Find challenge details
        challenge = next((c for c in test_plan["challenges"] if c["challenge_id"] == session["challenge_id"]), None)
        if not challenge:
    ...
    def _calculate_condition_metrics(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        development_times = [r["metrics"]["development_time_seconds"] for r in results if r["metrics"]["development_time_seconds"]]
        debug_iterations = [r["debug_iterations"] for r in results]
        
        success_rate = sum(1 for r in results if r["success_criteria_met"]["overall_success"]) / len(results)
        ...
        return {
            ...
            "development_time": {
                "mean": sum(development_times) / len(development_times) if development_times else 0,
    ...
    def _calculate_improvements(self, baseline: Dict[str, Any], mcp: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if baseline["overall_score"]["mean"] > 0:
            improvements["overall_score_improvement"] = (
                ...
                baseline["overall_score"]["mean"] * 100
            )
        
        # Success rate improvement
        improvements["success_rate_improvement"] = (
            mcp["functional_score"]["success_rate"] - baseline["functional_score"]["success_rate"]
        ) * 100
        
        # Code quality improvement
        if baseline["code_quality_score"]["mean"] > 0:
            improvements["code_quality_improvement"] = (
                (mcp["code_quality_score"]["mean"] - baseline["code_quality_score"]["mean"]) /
                baseline["code_quality_score"]["mean"] * 100
            )
...
def main():
    ...
    # Print instructions
    print("\n📋 NEXT STEPS:")
    print("1. Review the generated challenges in: testing/datasets/ab_test_challenges.json")
