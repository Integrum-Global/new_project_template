...
class FixSuggestionEngine:
        ...
        for error in errors:
            ...
            if error_code in self.error_patterns:
                ...
                if suggestion:
                    ...
                    suggestion_key = (suggestion["error_code"], suggestion["description"])
                ...
                suggestion_key = (generic_suggestion["error_code"], generic_suggestion["description"])
                if suggestion_key not in seen_suggestions:
    ...
    def _suggest_add_get_parameters(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        node_type = (error.get("node_type") or 
                    error.get("node_name") or 
                    error.get("node_class") or 
                    "YourNode")
...
NodeParameter(
    ...
    def _suggest_add_required_parameter(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        param_name = (error.get("parameter_name") or 
                     error.get("parameter") or 
                     "missing_param")
        
        # Try multiple field names that might contain the node name
        node_id = (error.get("node_name") or 
                  error.get("node_id") or 
                  "node")
...
workflow.add_node("{error.get('node_type', 'NodeType')}", "{node_id}", {{
    ...
    def _suggest_fix_connection_args(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "fix": f"Use 4 parameters for connection",
...
cycle_builder.build()''',
    ...
    def _suggest_fix_convergence_condition(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "code_example": f'''# ✅ VALID convergence conditions:
cycle_builder.converge_when("quality > 0.95")
...
cycle_builder.build()''',
    ...
    def _suggest_fix_cycle_mapping(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "code_example": f'''# ✅ CORRECT mapping format:
...
# cycle_builder.connect("node1", "node2", mapping=["list", "format"])''',
    ...
    def _suggest_fix_cycle_timeout(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "code_example": f'''# ✅ VALID timeout values:
...
# cycle_builder.timeout("5m")  # String values not allowed''',
    ...
    def _suggest_fix_cycle_node_reference(self, error: Dict[str, Any]) -> Dict[str, Any]:
...
# Make sure all referenced nodes exist before creating cycles''',
    ...
    def _suggest_add_missing_import(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        import_statement = import_statements.get(missing_name, f"# Add appropriate import for '{missing_name}'")
        
        return {
            "error_code": "IMP001",
            "description": f"Add missing import for '{missing_name}'",
            "fix": f"Add import statement at the top of your file",
...
results, run_id = runtime.execute(workflow.build())''',
    ...
    def _suggest_remove_unused_import(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "code_example": f'''# ❌ WRONG - Unused import:
...
from kailash.nodes.base import Node, NodeParameter''',
    ...
    def _suggest_fix_relative_import(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "error_code": "IMP004",
            "description": f"Convert relative import to absolute import",
            "fix": f"Use absolute import instead of relative import for '{import_name}'",
            "code_example": f'''# ❌ WRONG - Relative imports:
...
from kailash.workflow.validation import ParameterDeclarationValidator''',
            "explanation": f"Relative imports (starting with . or ..) can cause import issues. Use absolute imports starting with 'kailash' for better compatibility and clarity."
        }
...
from .my_custom_module import CustomClass''',
    ...
    def _suggest_optimize_heavy_import(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "code_example": f'''# ❌ WRONG - Heavy import not used:
