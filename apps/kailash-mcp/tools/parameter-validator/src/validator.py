...
class ParameterValidator:
    ...
    def validate_workflow(self, workflow_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cycle_errors, cycle_warnings = self._validate_cycle_patterns(workflow_info, tree)
            ...
            if import_result.get("optimization_suggestions"):
                result["optimization_suggestions"] = import_result["optimization_suggestions"]
            ...
            for class_node in classes:
                ...
                if not has_get_parameters:
                    ...
                    if get_params_method:
                        ...
                        param_errors = self._validate_get_parameters_method(get_params_method, class_name)
                        errors.extend(param_errors)
    ...
    def _validate_get_parameters_method(self, method_node: ast.FunctionDef, class_name: str) -> List[Dict[str, Any]]:
        ...
        for node in ast.walk(method_node):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == "NodeParameter":
                ...
                if not param_name and len(node.args) >= 1 and isinstance(node.args[0], ast.Constant):
        ...
        try:
            ...
            if hasattr(self, '_current_class_node'):
                class_methods = [node for node in self._current_class_node.body 
    ...
    def validate_connections_only(self, connections: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                ...
                # Look for create_cycle calls
                      ...
                      node.func.attr == "create_cycle"):
                    cycle_info = self._extract_cycle_info(node)
                    if cycle_info:
                        workflow_info["cycles"].append(cycle_info)
        
        return workflow_info if workflow_info["nodes"] or workflow_info["connections"] or workflow_info["cycles"] else None
    ...
    def _extract_add_node_info(self, node: ast.Call) -> Optional[Dict[str, Any]]:
        ...
        try:
            node_type = ast.literal_eval(node.args[0]) if isinstance(node.args[0], ast.Constant) else None
            node_id = ast.literal_eval(node.args[1]) if isinstance(node.args[1], ast.Constant) else None
    ...
    def _validate_node_parameters(self, workflow_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for node_info in workflow_info["nodes"]:
            ...
            required_params = self._get_required_parameters(node_type, STATIC_REQUIRED_PARAMS)
    ...
    def _get_required_parameters(self, node_type: str, static_params: Dict[str, List[str]]) -> List[str]:
        """
    ...
    def _validate_connections(self, workflow_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for conn in workflow_info["connections"]:
            if conn.get("syntax_error") == "old_2_param":
                ...
                if output_field and ("nonexistent" in output_field.lower() or 
                                   "invalid" in output_field.lower() or
                                   "fake" in output_field.lower()):
                    errors.append({
                        "code": "CON006",
                ...
                if input_field and ("nonexistent" in input_field.lower() or 
                                  "invalid" in input_field.lower() or
                                  "fake" in input_field.lower()):
                    errors.append({
                        "code": "CON007", 
    ...
    def _validate_node_existence(self, workflow_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Validate that connected nodes exist."""
        errors = []
        
        # Get all node IDs
        node_ids = {node["node_id"] for node in workflow_info["nodes"] if node.get("node_id")}
    ...
    def _validate_no_circular_dependencies(self, workflow_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for node in graph:
            if node not in visited:
                if has_cycle(node):
                    errors.append({
                        "code": "CON005",
                        "message": f"Circular dependency detected in workflow connections",
                        "severity": "error"
                    })
                    break
    ...
    def _validate_cycle_patterns(self, workflow_info: Dict[str, Any], tree: ast.AST) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        cycle_errors, cycle_warnings = self._validate_cycle_builders(workflow_info, tree)
        errors.extend(cycle_errors)
        warnings.extend(cycle_warnings)
    ...
    def _check_deprecated_cycle_syntax(self, tree: ast.AST) -> List[Dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.attr == "add_connection":
                # Check for cycle keyword argument
                for keyword in node.keywords:
                    if keyword.arg == "cycle":
                        errors.append({
                            "code": "CYC001",
                            "message": f"Deprecated 'cycle=True' parameter detected. Use workflow.create_cycle() API instead",
                            "line": node.lineno,
                            "severity": "error"
                        })
    ...
    def _validate_cycle_builders(self, workflow_info: Dict[str, Any], tree: ast.AST) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        for cycle_name, cycle_data in cycle_builders.items():
            ...
            connection_errors = self._validate_cycle_connections(cycle_name, cycle_data, workflow_info)
            errors.extend(connection_errors)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                ...
                if (len(node.targets) == 1 and 
                    ...
                    node.value.func.attr == "create_cycle"):
                        ...
                        if cycle_data["variable"] == var_name:
                            if method_name == "connect":
                                ...
                                if len(node.value.args) >= 1:
                                    try:
                                        cycle_data["max_iterations"] = ast.literal_eval(node.value.args[0])
                                    except:
                                        cycle_data["max_iterations"] = "invalid"
                            elif method_name == "converge_when":
                                if len(node.value.args) >= 1:
                                    try:
                                        cycle_data["converge_when"] = ast.literal_eval(node.value.args[0])
                                    except:
                                        cycle_data["converge_when"] = "invalid"
                            elif method_name == "timeout":
                                if len(node.value.args) >= 1:
                                    try:
                                        cycle_data["timeout"] = ast.literal_eval(node.value.args[0])
                                    except:
    ...
    def _validate_cycle_configuration(self, cycle_name: str, cycle_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if cycle_data["converge_when"] == "invalid":
            ...
            if ("!" in condition and not any(op in condition for op in ["!=", ">=", "<="])) or \
               condition.strip().startswith("invalid"):
                errors.append({
                    "code": "CYC003", 
        ...
        if cycle_data["timeout"] is not None:
            ...
            elif isinstance(cycle_data["timeout"], (int, float)) and cycle_data["timeout"] <= 0:
                errors.append({
                    "code": "CYC007",
    ...
    def _validate_cycle_connections(self, cycle_name: str, cycle_data: Dict[str, Any], workflow_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Validate cycle connections."""
        errors = []
    ...
    def _check_cycle_warnings(self, cycle_name: str, cycle_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Check for cycle configuration warnings."""
        warnings = []
        
        # Check for excessive iterations
        if (isinstance(cycle_data["max_iterations"], int) and 
            cycle_data["max_iterations"] > 1000):
            warnings.append({
                "code": "CYC006",
    ...
    def validate_imports(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_errors, missing_suggestions = self._validate_missing_imports(used_names, imports)
            ...
            performance_warnings = self._validate_import_performance(imports, used_names)
            ...
            optimization_suggestions = self._generate_optimization_suggestions(imports, used_names)
    ...
    def _validate_missing_imports(self, used_names: Set[str], imports: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    def _validate_unused_imports(self, imports: List[Dict[str, Any]], used_names: Set[str]) -> List[Dict[str, Any]]:
    ...
    def _validate_import_paths(self, imports: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Check for incorrect import paths."""
        errors = []
    ...
    def _validate_relative_imports(self, imports: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _validate_import_ordering(self, imports: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for imp in imports:
            ...
            if imp_group != expected_group and imp["line"] > previous_line:
                ...
                imp_group_index = expected_order.index(imp_group) if imp_group in expected_order else len(expected_order)
    ...
    def _validate_import_performance(self, imports: List[Dict[str, Any]], used_names: Set[str]) -> List[Dict[str, Any]]:
        """Check for performance-impacting unused imports."""
        warnings = []
        
        # Heavy imports that can impact startup time
        heavy_imports = {
            "tensorflow", "tf", "torch", "pandas", "pd", "numpy", "np",
            "matplotlib", "plt", "sklearn", "cv2", "PIL", "transformers"
        }
        ...
        for imp in imports:
            ...
            is_heavy = (import_name in heavy_imports or 
                       any(part in heavy_imports for part in module_parts))
    ...
    def _generate_optimization_suggestions(self, imports: List[Dict[str, Any]], used_names: Set[str]) -> List[Dict[str, Any]]:
        ...
        for imp in from_imports:
            if imp["module"] not in modules:
                modules[imp["module"]] = []
            modules[imp["module"]].append(imp["original_name"] if "original_name" in imp else imp["name"])
    ...
    def _is_standard_library(self, module: str) -> bool:
        ...
        stdlib_modules = {
            ...
            "datetime", "time", "math", "random", "string", "io", "urllib",
            "http", "email", "html", "xml", "csv", "sqlite3", "pickle",
            "unittest", "logging", "argparse", "configparser", "threading",
            "multiprocessing", "asyncio", "concurrent", "queue", "socket",
            "ssl", "hashlib", "hmac", "secrets", "base64", "binascii",
            "struct", "array", "copy", "pprint", "reprlib", "enum", "types",
            "weakref", "gc", "inspect", "dis", "importlib", "pkgutil",
            "modulefinder", "runpy", "site", "sysconfig", "platform",
            "errno", "ctypes", "abc", "contextlib", "warnings", "trace"
        }
    ...
    def analyze_complexity(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_suggestions = self._generate_complexity_optimizations(workflow_info, metrics)
    ...
    def _calculate_complexity_metrics(self, workflow_info: Dict[str, Any], tree: ast.AST) -> Dict[str, Any]:
        ...
        maintenance_complexity = (
            ...
            config_complexity * 20 +
            connection_complexity * 15
        )
    ...
    def _determine_pattern_type(self, workflow_info: Dict[str, Any]) -> str:
        ...
        is_linear = all(count <= 1 for count in incoming_counts.values()) and \
                   all(count <= 1 for count in outgoing_counts.values())
        
        if is_linear:
            return "linear"
        
        # Check for parallel pattern (multiple nodes with same source/target)
        has_parallel = any(count > 1 for count in outgoing_counts.values()) or \
    ...
    def _calculate_parallelism_score(self, workflow_info: Dict[str, Any]) -> float:
        ...
        parallel_connections = sum(count - 1 for count in outgoing_counts.values() if count > 1)
    ...
    def _calculate_configuration_complexity(self, nodes: List[Dict[str, Any]]) -> float:
        ...
        for node in nodes:
            ...
            nested_complexity = sum(
                1 if isinstance(v, (dict, list)) else 0
    ...
    def _calculate_connection_complexity(self, connections: List[Dict[str, Any]]) -> float:
    ...
    def _detect_performance_bottlenecks(self, workflow_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        high_latency_nodes = [
            ...
            if node.get("node_type") in high_latency_types
        ]
    ...
    def _detect_error_risks(self, workflow_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        external_node_types = {"HTTPRequestNode", "DatabaseQueryNode", "ExternalAPINode"}
        external_nodes = [
            node for node in nodes
            if node.get("node_type") in external_node_types
        ]
        
        for node in external_nodes:
            config = node.get("parameters", {})
            has_retry = any(key in config for key in ["retry", "max_retries", "retry_config"])
        ...
        critical_nodes = [node_id for node_id, count in outgoing_counts.items() if count >= 3]
    ...
    def _generate_complexity_optimizations(self, workflow_info: Dict[str, Any], metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if metrics.get("pattern_type") == "linear" and metrics.get("node_count", 0) >= 4:
            suggestions.append({
        ...
        for node_type, count in node_types.items():
            if count >= 3 and node_type in {"LLMAgentNode", "HTTPRequestNode", "DatabaseQueryNode"}:
                suggestions.append({
                    "type": "add_caching",
                    "priority": "medium",
    ...
    def _analyze_resource_usage(self, workflow_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        memory_intensive = {"LLMAgentNode", "EmbeddingGeneratorNode", "VectorSearchNode", "MLModelNode"}
        cpu_intensive = {"DataProcessorNode", "MLModelNode", "ImageProcessorNode", "ComputeNode"}
        
        memory_intensive_count = sum(
            1 for node in nodes 
            if node.get("node_type") in memory_intensive
        )
        
        cpu_intensive_count = sum(
            1 for node in nodes
            if node.get("node_type") in cpu_intensive
        )
        ...
        return {
            ...
            "resource_efficiency_score": min(1.0, len(nodes) / max(1, memory_intensive_count + cpu_intensive_count))
        }
    ...
    def _analyze_scalability(self, workflow_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        has_load_balancer = any(
            "LoadBalancer" in node.get("node_type", "") or "Balancer" in node.get("node_type", "")
            for node in nodes
        )
        
        has_workers = sum(
            1 for node in nodes
            if "Worker" in node.get("node_type", "") or "Processor" in node.get("node_type", "")
        )
        ...
        avg_outgoing = sum(outgoing_counts.values()) / len(outgoing_counts) if outgoing_counts else 1
        ...
        horizontal_scaling_potential = min(1.0, parallel_score / max(1, len(nodes) // 2))
        ...
        return {
            ...
            "scalability_score": round((horizontal_scaling_potential + load_distribution_score) / 2, 2)
        }