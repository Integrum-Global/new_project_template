...
class ParameterValidationTools:
    ...
    def register_tools(self, server):
        ...
        @server.tool()
        def validate_gold_standards(code: str, check_type: str = "all") -> Dict[str, Any]:
            """
            ...
            try:
                ...
                result = {
                    ...
                    "compliance_score": max(0, 100 - (len(errors) * 10) - (len(warnings) * 5))
                }
    ...
    def run(self, **kwargs):
        return {"result": "value"}''',
                ...
                return {
                    "patterns": patterns,
                    "total_patterns": len(patterns["parameter_patterns"]) + len(patterns["common_mistakes"]),
        ...
        def check_error_pattern(code: str, pattern_type: str) -> Dict[str, Any]:
            ...
            try:
                if pattern_type == "connection_syntax":
                    ...
                    workflow_info = self.validator._extract_workflow_info(ast.parse(code))
                    if workflow_info:
                        ...
                        return {
                            ...
                            "has_errors": len(errors) > 0,
                            "errors": errors,
                            "suggestions": self.suggestion_engine.generate_fixes(errors) if errors else []
                        }
                    ...
                    return {
                        ...
                        "suggestions": self.suggestion_engine.generate_fixes(result["errors"]) if result["has_errors"] else []
                    }
                    ...
                    workflow_info = self.validator._extract_workflow_info(ast.parse(code))
                    if workflow_info:
                        errors = self.validator._validate_no_circular_dependencies(workflow_info)
                        return {
                            "pattern_type": pattern_type,
                            "has_errors": len(errors) > 0,
                            "errors": errors,
    ...
    def _is_valid_pattern_uri(self, uri: str) -> bool:
        ...
        if not uri.startswith("patterns://"):
            return False
        parts = uri.split("/")
        return len(parts) >= 4 and parts[2] != ""  # Must have at least patterns://category/item
        ...
        try:
            ...
            if all_contents:  # If we have mocked contents, use them
                for filename, description in all_contents.items():
                    ...
                    common_terms = sum(1 for term in key_terms if term.lower() in description.lower())
                    if common_terms >= 1:  # Lower threshold for testing
                        related.append({
                            "name": pattern_name_from_file,
                            "filename": filename,
                            "similarity_score": common_terms,
                            "category": self._extract_category_from_name(pattern_name_from_file)
                        })
            else:
                ...
                for pattern in all_patterns:
                    ...
                    if content:
                        # Count common terms
                        common_terms = sum(1 for term in key_terms if term.lower() in content.lower())
    ...
    def _get_all_pattern_contents(self) -> Dict[str, str]:
        ...
        for pattern in patterns:
            content = self.get_pattern_content(pattern["name"])
            if content:
                contents[pattern["filename"]] = self._extract_description_from_content(content)
    ...
    def _extract_key_terms(self, content: str) -> List[str]:
        ...
        terms = []
        common_terms = [
            "WorkflowBuilder", "LLMAgentNode", "HTTPRequestNode", "LocalRuntime",
            "cycle", "connection", "parameter", "workflow", "node", "validation",
            "MCP", "agent", "data", "process", "execute", "build"
        ]
