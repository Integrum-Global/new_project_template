...
async def test_server_lifecycle():
    ...
    with patch.object(server.mcp_server, 'run_stdio', new_callable=AsyncMock) as mock_run_stdio:
        await server.start()
...
'''
    ...
    suggestions = server.validation_tools.suggestion_engine.generate_fixes(result1["errors"])
...
async def test_performance_with_large_workflow():
    ...
    # Add 50 nodes
    for i in range(50):
        large_workflow_parts.append(f'workflow.add_node("ProcessorNode", "node_{i}", {{"operation": "process_{i}"}})')
    
    # Add connections between nodes
    for i in range(49):
        large_workflow_parts.append(f'workflow.add_connection("node_{i}", "result", "node_{i+1}", "input")')
