...
def test_validate_connections_schema(validation_tools, mcp_server):
    ...
    connections_tool = next((t for t in tools if t["name"] == "validate_connections"), None)
...
def test_tool_execution_response_format(validation_tools):
    ...
    if result["has_errors"]:
        suggestions = validation_tools.suggestion_engine.generate_fixes(result["errors"])
...
def test_json_serialization(validation_tools):
    ...
    result = validation_tools.validator.validate_workflow("from kailash.workflow.builder import WorkflowBuilder")
...
def test_parameter_schemas_json_types(validation_tools, mcp_server):
    ...
    for tool in tools:
        ...
        for prop_name, prop_schema in schema.get("properties", {}).items():
            ...
            assert prop_schema["type"] in ["string", "number", "integer", "boolean", "array", "object", "null"]
            ...
            if prop_schema["type"] == "object":
                assert "properties" in prop_schema or "additionalProperties" in prop_schema
...
def test_tool_descriptions(validation_tools, mcp_server):
    ...
    for tool in tools:
        ...
        assert "validate" in tool["description"].lower() or "check" in tool["description"].lower() or "suggest" in tool["description"].lower()
...
def test_tool_name_conventions(validation_tools, mcp_server):
    ...
    core_tools = {"validate_workflow", "check_node_parameters", "validate_connections", "suggest_fixes"}
    ...
    for tool_name in actual_tools:
        ...
        assert tool_name.replace("_", "").isalnum() or tool_name.replace("_", "").replace("-", "").isalnum()
