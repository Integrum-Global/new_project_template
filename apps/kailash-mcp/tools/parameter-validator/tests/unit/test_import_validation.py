...
'''
    ...
    wb_suggestions = [s for s in suggestions if "WorkflowBuilder" in s["import_statement"]]
    assert len(wb_suggestions) == 1
    assert "from kailash.workflow.builder import WorkflowBuilder" in wb_suggestions[0]["import_statement"]
    ...
    assert "from kailash.runtime.local import LocalRuntime" in lr_suggestions[0]["import_statement"]
...
'''
    ...
    if "optimization_suggestions" in result:
        ...
        consolidation_suggestions = [s for s in suggestions if s["type"] == "consolidate"]
        if consolidation_suggestions:
            assert "from kailash.runtime import LocalRuntime, ParallelRuntime" in str(consolidation_suggestions)
...
'''
    ...
    if heavy_import_warnings:
        assert any("tensorflow" in w["message"] or "torch" in w["message"] 
...
'''
    ...
    assert any(prefix in errors_by_code for prefix in ["PAR", "CON"])  # Other validation errors