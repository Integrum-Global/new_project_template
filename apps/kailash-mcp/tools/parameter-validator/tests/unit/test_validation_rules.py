...
def test_par001_empty_parameters(parameter_validator, node_with_missing_parameters_code):
    """Test PAR001: Node missing get_parameters() method."""
    result = parameter_validator.validate_node_parameters(node_with_missing_parameters_code)
...
'''
    ...
    assert expected_codes.intersection(error_codes), f"Expected codes {expected_codes}, got {error_codes}"
...
def test_valid_node_parameters(parameter_validator, node_with_valid_parameters_code):
    ...
    result = parameter_validator.validate_node_parameters(node_with_valid_parameters_code)
...
def test_valid_workflow_parameters(parameter_validator, valid_workflow_code):
    ...
    critical_errors = [e for e in result["errors"] if e["severity"] == "error" and e["code"].startswith("PAR")]
