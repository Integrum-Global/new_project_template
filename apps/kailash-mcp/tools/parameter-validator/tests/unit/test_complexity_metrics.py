...
'''
    ...
    assert result["metrics"]["max_cycle_depth"] >= 0  # May be 0 if cycle extraction differs
...
'''
    ...
    parallel_suggestions = [s for s in suggestions if s["type"] == "parallelize_operations"]
    assert len(parallel_suggestions) >= 1
    
    # Should suggest pipeline optimization
    pipeline_suggestions = [s for s in suggestions if s["type"] == "pipeline_optimization"]
...
'''
    ...
    assert scalability["horizontal_scaling_potential"] >= 0.5  # More realistic threshold
