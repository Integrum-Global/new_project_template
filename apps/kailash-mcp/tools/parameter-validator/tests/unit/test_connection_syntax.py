...
'''
    ...
    assert "2-parameter" in con002_errors[0]["message"] or "old syntax" in con002_errors[0]["message"]
...
'''
    ...
    field_errors = [e for e in result["errors"] if "field" in e["message"].lower() or "output" in e["message"].lower() or "input" in e["message"].lower()]
...
'''
    ...
    assert len(connection_errors) == 0, f"Unexpected connection errors: {connection_errors}"
