...
def test_parameter_fix_suggestion(suggestion_engine):
    ...
    par001_suggestion = next((s for s in suggestions if s["error_code"] == "PAR001"), None)
...
def test_connection_fix_suggestion(suggestion_engine):
    ...
    assert len(suggestions) >= 1
    con002_suggestion = next((s for s in suggestions if s["error_code"] == "CON002"), None)
    assert con002_suggestion is not None
    
    # Should suggest 4-parameter syntax
    assert "4 parameters" in con002_suggestion["description"] or "4-parameter" in con002_suggestion["description"]
    assert 'add_connection("node1", "result", "node2", "input")' in con002_suggestion["code_example"]
...
def test_type_fix_suggestion(suggestion_engine):
    ...
    assert len(suggestions) >= 1
    par003_suggestion = next((s for s in suggestions if s["error_code"] == "PAR003"), None)
...
def test_par004_missing_required_suggestion(suggestion_engine):
    ...
    par004_suggestion = next((s for s in suggestions if s["error_code"] == "PAR004"), None)
...
def test_con003_nonexistent_source_suggestion(suggestion_engine):
    ...
    con003_suggestion = next((s for s in suggestions if s["error_code"] == "CON003"), None)
    assert con003_suggestion is not None
    
    # Should suggest creating the node or fixing the reference
    assert "add_node" in con003_suggestion["code_example"] or "nonexistent" in con003_suggestion["fix"]
...
def test_con004_nonexistent_target_suggestion(suggestion_engine):
    ...
    con004_suggestion = next((s for s in suggestions if s["error_code"] == "CON004"), None)
    assert con004_suggestion is not None
    
    # Should suggest creating the target node
    assert "add_node" in con004_suggestion["code_example"] or "missing_target" in con004_suggestion["fix"]
...
def test_con005_circular_dependency_suggestion(suggestion_engine):
    ...
    con005_suggestion = next((s for s in suggestions if s["error_code"] == "CON005"), None)
    ...
    assert "remove" in con005_suggestion["fix"].lower() or "restructure" in con005_suggestion["fix"].lower()
...
def test_gold_standard_suggestions(suggestion_engine):
    ...
    gold_suggestion = next((s for s in suggestions if s["error_code"] == "GOLD002"), None)
...
def test_contextual_suggestions(suggestion_engine):
    ...
    assert "LLMAgentNode" in llm_suggestion["code_example"] or "LLM" in llm_suggestion["description"]
...
def test_code_example_syntax(suggestion_engine, validation_error_examples):
    ...
    for suggestion in suggestions:
        ...
        # Basic syntax checks
        assert code_example.count('(') == code_example.count(')')  # Balanced parentheses
        ...
        assert 'def ' not in code_example or ':' in code_example  # Function definitions have colons
