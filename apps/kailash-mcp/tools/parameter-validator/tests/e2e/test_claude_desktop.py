...
''',
    ...
    for test_case in test_cases:
        ...
        assert result["has_errors"], f"Should detect errors for: {test_case['description']}"
        
        # Verify expected error codes are present
        error_codes = {e["code"] for e in result["errors"]}
        for expected_code in test_case["expected_errors"]:
            assert expected_code in error_codes, f"Missing {expected_code} for {test_case['description']}"
...
''' + '\n'.join([
    ...
    assert validation_time < 0.5, f"Validation took {validation_time:.3f}s, should be <0.5s"
...
async def test_concurrent_usage():
    ...
    assert total_time < 2.0, f"Concurrent validation took {total_time:.3f}s, should be <2s"
...
async def test_error_recovery():
    ...
    for problematic_input in problematic_inputs:
        try:
            result = server.validation_tools.validator.validate_workflow(problematic_input)
            ...
            pytest.fail(f"Tool crashed on input: {repr(problematic_input[:50])}... Error: {e}")
