...
async def test_validation_usage():
    ...
    with patch('kailash.mcp_server.MCPClient') as mock_mcp_client:
        ...
        workflow = WorkflowBuilder()
        workflow.add_node("LLMAgentNode", "code_reviewer", {
            "model": "gpt-4",
            "prompt": """
...
'''
        ...
        # Simulate the call
        result = param_server.validation_tools.validator.validate_workflow(workflow_code)
        
        # Verify it catches the error
        assert result["has_errors"]
        assert len(result["errors"]) >= 1
        assert any("parameter" in error["message"].lower() for error in result["errors"])
...
'''
    ...
    result1 = param_server.validation_tools.validator.validate_workflow(invalid_workflow)
    assert result1["has_errors"]
    initial_error_count = len(result1["errors"])
    
    # Generate suggestions
    suggestions = param_server.validation_tools.suggestion_engine.generate_fixes(result1["errors"])
...
'''
    ...
    result2 = param_server.validation_tools.validator.validate_workflow(corrected_workflow)
...
'''
    ...
    result = param_server.validation_tools.validator.validate_workflow(generated_workflow_code)
...
async def test_agent_collaboration_scenario():
    ...
    workflow.add_node("LLMAgentNode", "code_fixer", {
        "model": "gpt-4",
    ...
    # Connect the agents
    workflow.add_connection("code_generator", "result", "code_reviewer", "code_to_review")
...
'''
    ...
    result = param_server.validation_tools.validator.validate_workflow(agent_workflow_code)
    
    # Agent collaboration workflow should be valid
    critical_errors = [e for e in result["errors"] if e["severity"] == "error"]
    assert len(critical_errors) == 0, f"Agent collaboration workflow has errors: {critical_errors}"
...
''',
    ...
    assert total_time < 1.0, f"Agent validation rounds took {total_time:.3f}s, should be <1s"
    ...
    assert error_counts[-1] <= error_counts[0], "Workflow should improve through iterations"
...
async def test_edge_case_handling_in_agent_context():
    ...
    result1 = param_server.validation_tools.validator.validate_workflow(minimal_workflow)
...
'''
    ...
    result2 = param_server.validation_tools.validator.validate_workflow(complex_workflow)
...
'''
    ...
    result3 = param_server.validation_tools.validator.validate_workflow(workflow_with_imports)
...
async def test_integration_error_recovery():
    ...
    for scenario_name, problematic_code in failure_scenarios:
        try:
            result = param_server.validation_tools.validator.validate_workflow(problematic_code)
            ...
            assert isinstance(result["errors"], list), f"Errors not list for {scenario_name}"
