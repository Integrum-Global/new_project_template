...
'''
    ...
    assert len(detected_codes) >= 3, f"Expected multiple error types, got: {error_codes}"
    
    # Generate suggestions for all errors
    suggestions = server.validation_tools.suggestion_engine.generate_fixes(result["errors"])
...
'''
    ...
    initial_result = server.validation_tools.validator.validate_workflow(fixable_workflow)
    assert initial_result["has_errors"]
    
    suggestions = server.validation_tools.suggestion_engine.generate_fixes(initial_result["errors"])
...
'''
    ...
    assert len(resolved_codes.intersection(critical_codes)) >= 1  # At least one critical error resolved
...
'''
    ...
    expected_error_types = {"PAR004", "CON002", "CON004"}  # Missing params, wrong syntax, bad target
    
    detected_types = error_codes.intersection(expected_error_types)
    assert len(detected_types) >= 2, f"Should detect multiple error types, got: {error_codes}"
...
async def test_error_correction_performance():
    ...
    for i in range(20):
        large_workflow_parts.append(f'workflow.add_node("ProcessorNode", "node_{i}", {{}})')  # Missing parameters
    
    # Add problematic connections
    for i in range(19):
        large_workflow_parts.append(f'workflow.add_connection("node_{i}", "node_{i+1}")')  # Wrong syntax
    ...
    assert total_time < 2.0, f"Large workflow correction took {total_time:.3f}s, should be <2s"
