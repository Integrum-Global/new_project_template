...
class TestWorkflowRegistry:
    ...
    async def test_register_all_workflows(self, workflow_registry):
        ...
        workflow_registry.agent_ui.register_workflow_template = AsyncMock(return_value=mock_workflow_id)
...
"""
        ...
        results, _ = await test_runtime.execute(
            ...
            parameters={
                "input_data": {
                    "last_name": "User"  # Missing email and first_name
                }
            }
        )
...
"""
    ...
    async def test_workflow_error_handling(self, test_runtime):
        ...
        builder.add_node("PythonCodeNode", "error_node", {
            "name": "error_producer",
            "code": """
...
"""
        ...
        results, _ = await test_runtime.execute(
            workflow,
            parameters={"trigger_error": False}
        )
...
"""
        ...
        results, _ = await test_runtime.execute(
            workflow,
            parameters={"input_data": {"id": "test_123"}}
        )
...
"""
        ...
        tasks = []
        for i in range(5):
            task = test_runtime.execute(
                workflow,
                parameters={"input_id": f"test_{i}"}
            )
            tasks.append(task)
...
class TestWorkflowValidation:
    ...
    def test_workflow_node_validation(self):
        ...
        # Valid node
        builder.add_node("PythonCodeNode", "valid_node", {
            "name": "valid_processor",
            "code": "result = {'result': {'valid': True}}"
        })
