...
class TestUserEndpoints:
    ...
    async def test_create_user_success(self, test_client, sample_user_data, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            mock_agent_ui.execute_workflow_template = AsyncMock(
                return_value="test_execution"
            )
            mock_agent_ui.wait_for_execution = AsyncMock(
                return_value={
                    "outputs": {
                        "create_user": {
                            "user": {
            ...
            response = await test_client.post(
                "/api/users/", json=sample_user_data, headers=headers
            )
    ...
    async def test_create_user_invalid_data(self, test_client, auth_token):
        ...
        response = await test_client.post(
            "/api/users/", json=invalid_data, headers=headers
        )
    ...
    async def test_list_users(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            mock_agent_ui.create_session = AsyncMock(return_value="test_session")
            mock_agent_ui.create_dynamic_workflow = AsyncMock(
                return_value="test_workflow"
            )
            ...
            response = await test_client.get(
                "/api/users/?page=1&limit=50", headers=headers
            )
    ...
    async def test_get_user_not_found(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            mock_agent_ui.execute_workflow = AsyncMock(return_value="test_execution")
            mock_agent_ui.wait_for_execution = AsyncMock(
                return_value={"outputs": {"user_manager": {"user": None}}}
            )
    ...
    async def test_delete_user(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            response = await test_client.delete(
                f"/api/users/{user_id}?reason=Test deletion", headers=headers
            )
...
class TestAuthEndpoints:
    ...
    async def test_logout(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            mock_agent_ui.wait_for_execution = AsyncMock(
                return_value={
                    "outputs": {"invalidate_session": {"result": {"success": True}}}
                }
            )
...
class TestRoleEndpoints:
    ...
    async def test_list_roles(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            response = await test_client.get("/api/roles/", headers=headers)
    ...
    async def test_create_role(self, test_client, auth_token, sample_role_data):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            mock_agent_ui.wait_for_execution = AsyncMock(
                return_value={
                    "outputs": {
                        "validate_role": {"result": {"valid": True, "errors": []}},
                        "create_role": {
                            "role": {
                                "id": str(uuid.uuid4()),
            ...
            response = await test_client.post(
                "/api/roles/", json=sample_role_data, headers=headers
            )
    ...
    async def test_assign_role(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            response = await test_client.post(
                "/api/roles/assign", json=assignment_data, headers=headers
            )
...
class TestPermissionEndpoints:
    ...
    async def test_check_permission(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            response = await test_client.post(
                "/api/permissions/check", json=permission_data, headers=headers
            )
...
class TestAdminEndpoints:
    ...
    async def test_get_dashboard(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            response = await test_client.get("/api/admin/dashboard", headers=headers)
...
class TestAPIPerformance:
    ...
    async def test_concurrent_api_requests(self, test_client, auth_token):
        ...
        with patch("apps.user_management.core.startup.agent_ui") as mock_agent_ui:
            ...
            tasks = [test_client.get("/api/users/", headers=headers) for _ in range(5)]
