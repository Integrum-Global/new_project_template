...
async def test_user_management_with_qa_app():
    ...
    try:
        ...
        print(
            f"   ✅ Discovered {len(user_mgmt_analysis['security_features'])} security features"
        )
        print(
            f"   ✅ Discovered {len(user_mgmt_analysis['api_endpoints'])} API endpoints"
        )
        print(
            f"   ✅ Discovered {len(user_mgmt_analysis['cli_commands'])} CLI commands"
        )
        ...
        auto_personas = persona_manager.generate_personas_from_permissions(
            user_mgmt_analysis["permissions"]
        )
        ...
        key_personas = [
            ...
            if p.key in ["system_admin", "security_officer", "regular_user"]
        ]
        ...
        scenarios = scenario_generator.generate_scenarios_for_personas(
            top_personas, user_mgmt_analysis
        )
        ...
        for persona in top_personas:
            permissions_tested.update(
                persona.permissions
                if persona.permissions != ["*"]
                else user_mgmt_analysis["permissions"]
            )
        ...
        # Coverage analysis
        operations_coverage = (
            len(operations_covered) / len(user_mgmt_analysis["operations"]) * 100
        )
        permissions_coverage = (
            len(permissions_tested) / len(user_mgmt_analysis["permissions"]) * 100
        )
        interfaces_coverage = (
            len(interfaces_tested) / len(user_mgmt_analysis["interfaces"]) * 100
        )

        print("   📈 Test Coverage Analysis:")
        print(
            f"      • Operations Coverage: {operations_coverage:.1f}% ({len(operations_covered)}/{len(user_mgmt_analysis['operations'])})"
        )
        print(
            f"      • Permissions Coverage: {permissions_coverage:.1f}% ({len(permissions_tested)}/{len(user_mgmt_analysis['permissions'])})"
        )
        print(
            f"      • Interfaces Coverage: {interfaces_coverage:.1f}% ({len(interfaces_tested)}/{len(user_mgmt_analysis['interfaces'])})"
        )

        print(f"   🔒 Security Testing: {security_scenarios} security scenarios")
        print(
            f"   ⚡ Performance Testing: {performance_scenarios} performance scenarios"
        )
        print(f"   ⚙️  Functional Testing: {functional_scenarios} functional scenarios")

        # Quality metrics
        avg_expected_success = sum(p.expected_success_rate for p in top_personas) / len(
            top_personas
        )
        ...
        report_data = {
            ...
            "quality_metrics": {
                ...
                "comprehensive_coverage": (
                    operations_coverage + permissions_coverage + interfaces_coverage
                )
                / 3,
                "security_focus_percent": security_scenarios / len(scenarios) * 100,
                "performance_focus_percent": performance_scenarios
                / len(scenarios)
                * 100,
            },
        ...
        sys.path.insert(
            0, str(Path(__file__).parent.parent.parent.parent / "qa_agentic_testing")
        )
    ...
    if success_rate >= 80:
        ...
        print("   • Previous: Manual scenarios, limited personas, basic validation")
        print(
            f"   • Current: {len(final_personas)} auto-generated personas, {len(scenarios)} comprehensive scenarios"
        )
        print(
            f"   • Coverage: {operations_coverage:.1f}% operations, {permissions_coverage:.1f}% permissions"
        )
