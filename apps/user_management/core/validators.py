...
from kailash.templates.data.validation import ValidationResult as SDKValidationResult
...
class UserValidator(SecurityMixin, Node):
    ...
    def _register_validation_tools(self):
        ...
        @self.mcp_server.tool(
            cache_key="user_update_validation", cache_ttl=300, format_response="json"
        )
    ...
    def validate_create_data(self, user_data: Dict[str, Any]) -> SDKValidationResult:
        ...
        return SDKValidationResult(
            is_valid=is_valid, errors=all_errors, warnings=[], field_results={}
        )
    ...
    def validate_update_data(self, user_data: Dict[str, Any]) -> SDKValidationResult:
        ...
        all_errors = (
            types_result.errors + constraints_result.errors + patterns_result.errors
        )
        is_valid = len(all_errors) == 0
...
class RoleValidator(SecurityMixin, Node):
    ...
    def __init__(self):
        super().__init__()
        self.mcp_server = MCPServer(
            "role-validator", enable_cache=True, enable_metrics=True
        )
...
class SecurityValidator(SecurityMixin, Node):
    ...
    def validate_security_context(
        ...
        validation_rules = {
            ...
            "data_types": {"ip_address": str, "user_agent": str, "session_id": str},
            "patterns": {
        ...
        all_errors = (
            required_result.errors + types_result.errors + patterns_result.errors
        )
        is_valid = len(all_errors) == 0
