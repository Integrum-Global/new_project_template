...
""",
        ...
        list_data = (
            result.get("outputs", {}).get("filter_processor", {}).get("result", {})
        )
...
async def delete_user(
    user_id: str, reason: str = Query(..., description="Reason for deletion")
):
    """
    ...
    try:
        ...
        execution_id = await agent_ui.execute_workflow_template(
            ...
            inputs={"user_id": user_id, "deletion_reason": reason},
        )
        ...
        deletion_result = (
            result.get("outputs", {}).get("delete_user", {}).get("deletion_result", {})
        )
...
async def disable_user(
    user_id: str, reason: str = Query(..., description="Reason for disabling")
):
...
async def enable_user(user_id: str):
    ...
    try:
        ...
        execution_id = await agent_ui.execute_workflow_template(
            session_id,
            "user_update_enterprise",
            inputs={"user_id": user_id, "update_data": {"is_active": True}},
        )
