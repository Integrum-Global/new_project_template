    ...
    try:
        ...
        if "generate_id()" in content and "service" not in content.lower():
            results["issues"].append(
                "Implements ID generation instead of using service"
            )

        if "password_hash" in content and "SecurityService" not in content:
            results["issues"].append(
                "Implements password hashing instead of using SecurityService"
            )

        if "role_assignments = []" in content and "RoleService" not in content:
            results["issues"].append(
                "Implements role logic instead of using RoleService"
            )

        if results["inline_code_lines"] > 50:
            results["issues"].append(
                f"Excessive inline code ({results['inline_code_lines']} lines)"
            )
...
def print_analysis_report(workflow_type: str, analyses: List[Dict[str, any]]):
    ...
    for analysis in analyses:
        ...
        refactored = (
            "REFACTORED" if ("refactored" in analysis["filename"]) else "ORIGINAL"
        )
        ...
        print(
            f"  - Inline Code: {'Yes' if analysis['uses_inline_pythoncode'] else 'No'}"
        )
