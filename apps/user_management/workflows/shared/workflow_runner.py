...
class WorkflowRunner:
    ...
    def create_workflow(self, workflow_name: str) -> WorkflowBuilder:
        ...
        if self.enable_audit:
            builder.add_node("PythonCodeNode", "audit_start", {
...
"""
    ...
    def execute_workflow(self, 
                        ...
                        workflow_name: str = "unnamed_workflow") -> Tuple[Dict[str, Any], str]:
        """
        ...
        try:
            ...
            # Log execution for monitoring
            if self.enable_monitoring:
                self._log_execution_metrics(workflow_name, execution_time, False, str(e))
            
            raise WorkflowExecutionError(f"Workflow '{workflow_name}' failed: {str(e)}")
    
    def validate_workflow(self, workflow: Any, workflow_name: str = "unnamed_workflow") -> bool:
        """
        ...
        try:
            ...
            if not hasattr(workflow, 'nodes') or not workflow.nodes:
                print(f"❌ Workflow '{workflow_name}' validation failed: No nodes found")
                return False
...
def create_user_context_node(user_id: str, user_type: str, permissions: List[str]) -> Dict[str, Any]:
    """
...
def run_workflow_test(workflow_script_path: str, test_parameters: Optional[Dict[str, Any]] = None):
    """
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("workflow_module", workflow_script_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--test":
            ...
            runner = WorkflowRunner(user_type="admin", user_id="test_admin", enable_debug=True)
            ...
            if runner.validate_workflow(workflow, "test_workflow"):
                try:
                    results, execution_id = runner.execute_workflow(workflow, {}, "test_workflow")
            ...
            workflow_dirs = ["admin_workflows/scripts", "manager_workflows/scripts", "user_workflows/scripts"]
