...
class ExportRequest(BaseModel):
    ...
    format: str = Field(
        description="Export format: python or yaml", pattern="^(python|yaml)$"
    )
...
def add_studio_routes(gateway):
    ...
    if not hasattr(gateway, "ai_chat"):
        gateway.ai_chat = AIChatMiddleware(
            agent_ui_middleware=gateway.agent_ui, enable_semantic_search=True
        )
        ...
        try:
            ...
            if not session_id:
                session_id = await gateway.ai_chat.start_chat_session(
                    user_id="default_user", metadata={"source": "studio_api"}
                )
            ...
            response = await gateway.ai_chat.process_message(
                session_id=session_id, message=message, workflow_id=request.workflow_id
            )
            ...
            elif request.workflow_id and any(
                keyword in request.message.lower()
                for keyword in ["add", "modify", "change", "update", "remove"]
            ):
    ...
    async def export_workflow(
        ...
        session_id: str = Query(
            ..., description="Session ID required for workflow access"
        ),
    ):
        """
        ...
        try:
            ...
            return ExportResponse(
                format=request.format, content=content, filename=filename
            )
    ...
    @gateway.app.get(
        "/api/nodes/types", response_model=NodeTypesResponse, tags=["Schemas"]
    )
    ...
    async def workflow_live_updates(websocket, workflow_id: str):
        ...
        await gateway.realtime.handle_workflow_websocket(
            websocket=websocket, workflow_id=workflow_id
        )
...
def _generate_python_code(workflow: Workflow) -> str:
    ...
    for node_id, node_instance in workflow.nodes.items():
        ...
        if actual_node:
            ...
            lines.append(
                f'    workflow.add_node("{node_id}", {node_class}({config_str}))'
            )
        else:
            # Fallback for unknown node type
            lines.append(
                f'    # workflow.add_node("{node_id}", UnknownNode(name="{node_id}"))'
            )

    # Add connections
    if workflow.connections:
        lines.extend(["", "    # Add connections"])
...
def _get_node_category(node_name: str, node_class) -> str:
    ...
    # AI nodes
    if "ai" in module_path or any(
        name in node_name.lower()
        for name in ["llm", "agent", "embedding", "a2a", "mcp"]
    ):
        return "AI"

    # Data nodes
    elif "data" in module_path or any(
        name in node_name.lower()
        for name in ["csv", "json", "sql", "database", "reader", "sharepoint"]
    ):
        return "Data"

    # Logic nodes
    elif "logic" in module_path or any(
        name in node_name.lower()
        for name in ["switch", "merge", "convergence", "workflow"]
    ):
        return "Logic"

    # API nodes
    elif "api" in module_path or any(
        name in node_name.lower()
        for name in ["http", "rest", "oauth", "graphql", "api"]
    ):
        return "API"

    # Transform nodes
    elif "transform" in module_path or any(
        name in node_name.lower()
        for name in ["filter", "map", "transform", "chunk", "processor"]
    ):
        return "Transform"
    ...
    # Security nodes
    elif "security" in module_path or any(
        name in node_name.lower()
        for name in ["credential", "access", "audit", "security"]
    ):
        return "Security"

    # Admin nodes
    elif "admin" in module_path or any(
        name in node_name.lower()
        for name in ["user", "role", "permission", "management"]
    ):
        return "Admin"
