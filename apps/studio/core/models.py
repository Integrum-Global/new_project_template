...
class StudioWorkflow(BaseWorkflowModel):
    ...
    security_events = relationship(
        "SecurityEvent", back_populates="workflow", cascade="all, delete-orphan"
    )
    ...
    def add_connection(
        ...
        self.connections = self.connections + [
            connection
        ]  # Create new list for SQLAlchemy tracking
        self.updated_at = datetime.now(timezone.utc)
...
class WorkflowExecution(BaseExecutionModel):
    ...
    workflow_id = Column(
        String(255), ForeignKey("studio_workflows.workflow_id"), nullable=False
    )

    # Studio-specific relationships
    workflow = relationship("StudioWorkflow", back_populates="executions")
    security_events = relationship(
        "SecurityEvent", back_populates="execution", cascade="all, delete-orphan"
    )
...
class SecurityEvent(BaseSecurityEventModel):
    ...
    workflow_id = Column(
        String(255), ForeignKey("studio_workflows.workflow_id"), index=True
    )
    execution_id = Column(
        String(255), ForeignKey("studio_executions.execution_id"), index=True
    )
