...
from .models import (
    StudioWorkflow, WorkflowExecution, WorkflowTemplate, SecurityEvent,
    AuditLog, ComplianceAssessment, WorkflowStatus, ExecutionStatus, TemplateCategory
)
...
class StudioDatabase:
    ...
    async def initialize(self):
        ...
        try:
            ...
            async with self.manager.get_session() as session:
                result = await session.execute(select(func.count()).select_from(StudioWorkflow))
                count = result.scalar()
    ...
    async def get_workflow(self, workflow_id: str, tenant_id: str = "default") -> Optional[StudioWorkflow]:
    ...
    async def list_workflows(self, tenant_id: str = "default", 
                           ...
                           owner_id: str = None,
                           limit: int = 100,
                           offset: int = 0) -> List[StudioWorkflow]:
    ...
    async def save_execution(self, execution: WorkflowExecution) -> WorkflowExecution:
        ...
        async with self.manager.get_session(execution.tenant_id) as session:
            try:
                existing = await self.execution_repo.get(session, execution.execution_id)
    ...
    async def get_execution(self, execution_id: str, tenant_id: str = "default") -> Optional[WorkflowExecution]:
        """Get execution by ID."""
        async with self.manager.get_session(tenant_id) as session:
    ...
    async def list_executions(self, workflow_id: str = None, tenant_id: str = "default",
                            ...
                            limit: int = 100,
                            offset: int = 0) -> List[WorkflowExecution]:
    ...
    async def list_templates(self, category: TemplateCategory = None,
                           tenant_id: str = "default",
                           include_public: bool = True) -> List[WorkflowTemplate]:
    ...
    async def get_stats(self, tenant_id: str = "default") -> Dict[str, Any]:
        ...
        async with self.manager.get_session(tenant_id) as session:
            ...
            workflow_count = await session.execute(
                select(func.count()).select_from(StudioWorkflow).where(
                    and_(
                        StudioWorkflow.tenant_id == tenant_id,
                        StudioWorkflow.deleted_at.is_(None)
                    )
            ...
            workflow_by_status = await session.execute(
                select(StudioWorkflow.status, func.count()).select_from(StudioWorkflow).where(
                    and_(
                        StudioWorkflow.tenant_id == tenant_id,
                        StudioWorkflow.deleted_at.is_(None)
                    )
            ...
            execution_by_status = await session.execute(
                select(WorkflowExecution.status, func.count()).select_from(WorkflowExecution).where(
                    WorkflowExecution.tenant_id == tenant_id
            ...
            template_count = await session.execute(
                select(func.count()).select_from(WorkflowTemplate).where(
                    or_(
                        WorkflowTemplate.tenant_id == tenant_id,
                        WorkflowTemplate.is_public == True
                    )
            ...
            template_by_category = await session.execute(
                select(WorkflowTemplate.category, func.count()).select_from(WorkflowTemplate).where(
                    or_(
                        WorkflowTemplate.tenant_id == tenant_id,
                        WorkflowTemplate.is_public == True
                    )
    ...
    async def cleanup_old_executions(self, days: int = 30, tenant_id: str = "default") -> int:
