...
class WorkflowService:
    ...
    async def create_dynamic_workflow(
        self, session_id: str, config: Dict[str, Any], tenant_id: str = "default"
    ) -> str:
        ...
        await self.db.save_workflow(workflow)
        logger.info(
            f"Created dynamic workflow {workflow_id} with {len(workflow.nodes)} nodes"
        )
        return workflow_id

    async def execute_workflow(
        self, session_id: str, workflow_id: str, inputs: Dict[str, Any] = None
    ) -> str:
    ...
    async def update_workflow(
        self, workflow: StudioWorkflow, user_context: Dict[str, Any] = None
    ) -> StudioWorkflow:
        ...
        logger.info(
            f"Updated workflow {workflow.workflow_id} to version {workflow.version}"
        )
        return workflow
    ...
    async def generate_workflow_from_ai(
...
class ExecutionService:
    ...
    async def start_execution(
        ...
        session_id = workflow.session_id or await self.agent_ui.create_session(
            started_by or "anonymous"
        )
        execution_id = await self.agent_ui.execute_workflow(
            session_id, workflow_id, inputs or {}
        )
...
class NodeService:
    ...
    async def get_node_types(
        self, category: NodeType = None, user_context: Dict[str, Any] = None
    ) -> Dict[str, List[NodeSchema]]:
        ...
        for node_type, schema_data in node_types.items():
            ...
            mapped_category = category_mapping.get(
                raw_category, NodeType.CODE_EXECUTION
            )
    ...
    async def get_node_schema(
        self, node_type: str, user_context: Dict[str, Any] = None
    ) -> Optional[NodeSchema]:
    ...
    async def validate_node_config(
        ...
        required_params = [
            param
            for param, details in schema.parameters.items()
...
class TemplateService:
    ...
    async def get_templates(
        self, category: TemplateCategory = None, user_context: Dict[str, Any] = None
    ...
    async def create_workflow_from_template(
        ...
        logger.info(
            f"Created workflow {workflow.workflow_id} from template {template_id}"
        )
        return workflow
    ...
    async def _create_builtin_templates(self) -> List[WorkflowTemplate]:
        ...
        security_templates = [
            ...
            (
                "ABAC Permission Audit",
                "Comprehensive permission audit and compliance check",
            ),
            (
                "Security Event Analysis",
                "Automated security event analysis and alerting",
            ),
        ]
        ...
        data_templates = [
            (
                ...
                "Complete pipeline for CSV data processing with validation",
            ),
            (
            ...
            (
                ...
                "Automated file processing with error handling",
            ),
        ]
...
class ExportService:
    ...
    async def _export_production_style(self, workflow: StudioWorkflow) -> str:
        ...
        for node in workflow.nodes:
            lines.append(
                f'    builder.add_node("{node.node_type}", "{node.node_id}", {node.config})'
            )
