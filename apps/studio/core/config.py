...
class DatabaseConfig:
    ...
    def __post_init__(self):
        ...
        self.enable_migrations = (
            os.getenv("STUDIO_DB_MIGRATIONS", "true").lower() == "true"
        )
        self.backup_enabled = os.getenv("STUDIO_DB_BACKUP", "true").lower() == "true"
        self.backup_interval_hours = int(
            os.getenv("STUDIO_DB_BACKUP_INTERVAL", self.backup_interval_hours)
        )
...
class SecurityConfig:
    ...
    def __post_init__(self):
        ...
        self.jwt_expire_minutes = int(
            os.getenv("STUDIO_JWT_EXPIRE_MINUTES", self.jwt_expire_minutes)
        )
        self.jwt_refresh_expire_days = int(
            os.getenv("STUDIO_JWT_REFRESH_DAYS", self.jwt_refresh_expire_days)
        )

        # API key configuration
        self.api_key_enabled = (
            os.getenv("STUDIO_API_KEY_ENABLED", "true").lower() == "true"
        )
        self.api_key_header = os.getenv("STUDIO_API_KEY_HEADER", self.api_key_header)

        # Session and authentication
        self.session_timeout_minutes = int(
            os.getenv("STUDIO_SESSION_TIMEOUT", self.session_timeout_minutes)
        )
        self.max_login_attempts = int(
            os.getenv("STUDIO_MAX_LOGIN_ATTEMPTS", self.max_login_attempts)
        )
        self.lockout_duration_minutes = int(
            os.getenv("STUDIO_LOCKOUT_DURATION", self.lockout_duration_minutes)
        )

        # Password policy
        self.password_min_length = int(
            os.getenv("STUDIO_PASSWORD_MIN_LENGTH", self.password_min_length)
        )
        self.password_require_special = (
            os.getenv("STUDIO_PASSWORD_SPECIAL", "true").lower() == "true"
        )
        ...
        self.enable_audit_logging = (
            os.getenv("STUDIO_AUDIT_LOGGING", "true").lower() == "true"
        )
        self.enable_threat_detection = (
            os.getenv("STUDIO_THREAT_DETECTION", "true").lower() == "true"
        )
        self.enable_behavior_analysis = (
            os.getenv("STUDIO_BEHAVIOR_ANALYSIS", "true").lower() == "true"
        )
        self.abac_evaluation_timeout_ms = int(
            os.getenv("STUDIO_ABAC_TIMEOUT", self.abac_evaluation_timeout_ms)
        )

        # Compliance
        frameworks_env = os.getenv(
            "STUDIO_COMPLIANCE_FRAMEWORKS", ",".join(self.compliance_frameworks)
        )
        self.compliance_frameworks = [f.strip() for f in frameworks_env.split(",")]
        self.data_retention_days = int(
            os.getenv("STUDIO_DATA_RETENTION_DAYS", self.data_retention_days)
        )
...
class MiddlewareConfig:
    ...
    def __post_init__(self):
        ...
        self.agent_ui_enabled = (
            os.getenv("STUDIO_AGENT_UI_ENABLED", "true").lower() == "true"
        )
        self.realtime_enabled = (
            os.getenv("STUDIO_REALTIME_ENABLED", "true").lower() == "true"
        )
        self.api_gateway_enabled = (
            os.getenv("STUDIO_API_GATEWAY_ENABLED", "true").lower() == "true"
        )
        self.ai_chat_enabled = (
            os.getenv("STUDIO_AI_CHAT_ENABLED", "true").lower() == "true"
        )
        self.session_management_enabled = (
            os.getenv("STUDIO_SESSION_MGMT_ENABLED", "true").lower() == "true"
        )
        self.event_streaming_enabled = (
            os.getenv("STUDIO_EVENT_STREAMING_ENABLED", "true").lower() == "true"
        )

        # Cache configuration
        self.workflow_repository_cache_ttl = int(
            os.getenv("STUDIO_WORKFLOW_CACHE_TTL", self.workflow_repository_cache_ttl)
        )
        self.execution_repository_cache_ttl = int(
            os.getenv("STUDIO_EXECUTION_CACHE_TTL", self.execution_repository_cache_ttl)
        )

        # Performance settings
        self.max_concurrent_sessions = int(
            os.getenv("STUDIO_MAX_CONCURRENT_SESSIONS", self.max_concurrent_sessions)
        )
        self.websocket_heartbeat_interval = int(
            os.getenv("STUDIO_WS_HEARTBEAT_INTERVAL", self.websocket_heartbeat_interval)
        )
        self.sse_retry_timeout = int(
            os.getenv("STUDIO_SSE_RETRY_TIMEOUT", self.sse_retry_timeout)
        )
        self.middleware_timeout_seconds = int(
            os.getenv("STUDIO_MIDDLEWARE_TIMEOUT", self.middleware_timeout_seconds)
        )
...
class ExecutionConfig:
    ...
    def __post_init__(self):
        ...
        self.max_concurrent_executions = int(
            os.getenv("STUDIO_MAX_EXECUTIONS", self.max_concurrent_executions)
        )
        self.max_concurrent_per_tenant = int(
            os.getenv(
                "STUDIO_MAX_EXECUTIONS_PER_TENANT", self.max_concurrent_per_tenant
            )
        )
        self.default_timeout_seconds = int(
            os.getenv("STUDIO_EXECUTION_TIMEOUT", self.default_timeout_seconds)
        )
        self.max_timeout_seconds = int(
            os.getenv("STUDIO_MAX_EXECUTION_TIMEOUT", self.max_timeout_seconds)
        )
        self.cleanup_interval_minutes = int(
            os.getenv("STUDIO_CLEANUP_INTERVAL", self.cleanup_interval_minutes)
        )
        self.retain_completed_days = int(
            os.getenv("STUDIO_RETAIN_COMPLETED_DAYS", self.retain_completed_days)
        )
        self.retain_failed_days = int(
            os.getenv("STUDIO_RETAIN_FAILED_DAYS", self.retain_failed_days)
        )

        # Resource monitoring
        self.enable_resource_monitoring = (
            os.getenv("STUDIO_RESOURCE_MONITORING", "true").lower() == "true"
        )
        self.max_memory_mb = int(os.getenv("STUDIO_MAX_MEMORY_MB", self.max_memory_mb))
        self.max_cpu_percent = int(
            os.getenv("STUDIO_MAX_CPU_PERCENT", self.max_cpu_percent)
        )

        # Auto-scaling
        self.enable_auto_scaling = (
            os.getenv("STUDIO_AUTO_SCALING", "false").lower() == "true"
        )
        self.auto_scale_threshold = float(
            os.getenv("STUDIO_AUTO_SCALE_THRESHOLD", self.auto_scale_threshold)
        )

        # Execution queue
        self.enable_execution_queue = (
            os.getenv("STUDIO_EXECUTION_QUEUE", "true").lower() == "true"
        )
        self.queue_max_size = int(
            os.getenv("STUDIO_QUEUE_MAX_SIZE", self.queue_max_size)
        )

        # Priority levels
        priority_env = os.getenv(
            "STUDIO_PRIORITY_LEVELS", ",".join(self.priority_levels)
        )
...
class AIConfig:
    ...
    def __post_init__(self):
        ...
        # AI features
        self.enable_ai_generation = (
            os.getenv("STUDIO_AI_GENERATION", "true").lower() == "true"
        )
        self.enable_ai_chat = os.getenv("STUDIO_AI_CHAT", "true").lower() == "true"
        self.enable_ai_recommendations = (
            os.getenv("STUDIO_AI_RECOMMENDATIONS", "true").lower() == "true"
        )
        self.enable_ai_troubleshooting = (
            os.getenv("STUDIO_AI_TROUBLESHOOTING", "true").lower() == "true"
        )

        # AI parameters
        self.ai_timeout_seconds = int(
            os.getenv("STUDIO_AI_TIMEOUT", self.ai_timeout_seconds)
        )
        ...
        self.frequency_penalty = float(
            os.getenv("STUDIO_AI_FREQUENCY_PENALTY", self.frequency_penalty)
        )
        self.presence_penalty = float(
            os.getenv("STUDIO_AI_PRESENCE_PENALTY", self.presence_penalty)
        )

        # Advanced AI features
        self.enable_streaming = (
            os.getenv("STUDIO_AI_STREAMING", "true").lower() == "true"
        )
        self.enable_function_calling = (
            os.getenv("STUDIO_AI_FUNCTION_CALLING", "true").lower() == "true"
        )
        self.enable_embeddings = (
            os.getenv("STUDIO_AI_EMBEDDINGS", "true").lower() == "true"
        )
        self.embedding_model = os.getenv(
            "STUDIO_AI_EMBEDDING_MODEL", self.embedding_model
        )

        # Vector database
        self.vector_db_enabled = (
            os.getenv("STUDIO_VECTOR_DB_ENABLED", "false").lower() == "true"
        )
...
class MonitoringConfig:
    ...
    def __post_init__(self):
        ...
        self.metrics_enabled = (
            os.getenv("STUDIO_METRICS_ENABLED", "true").lower() == "true"
        )
        self.health_check_enabled = (
            os.getenv("STUDIO_HEALTH_CHECK_ENABLED", "true").lower() == "true"
        )
        self.performance_monitoring_enabled = (
            os.getenv("STUDIO_PERFORMANCE_MONITORING", "true").lower() == "true"
        )
        ...
        self.enable_structured_logging = (
            os.getenv("STUDIO_STRUCTURED_LOGGING", "true").lower() == "true"
        )
        self.enable_request_logging = (
            os.getenv("STUDIO_REQUEST_LOGGING", "true").lower() == "true"
        )
        self.enable_error_tracking = (
            os.getenv("STUDIO_ERROR_TRACKING", "true").lower() == "true"
        )
        self.enable_performance_profiling = (
            os.getenv("STUDIO_PERFORMANCE_PROFILING", "false").lower() == "true"
        )

        # Metrics configuration
        self.metrics_port = int(os.getenv("STUDIO_METRICS_PORT", self.metrics_port))
        self.health_check_path = os.getenv(
            "STUDIO_HEALTH_CHECK_PATH", self.health_check_path
        )
        self.metrics_path = os.getenv("STUDIO_METRICS_PATH", self.metrics_path)
        self.prometheus_enabled = (
            os.getenv("STUDIO_PROMETHEUS_ENABLED", "false").lower() == "true"
        )
        self.grafana_dashboard_enabled = (
            os.getenv("STUDIO_GRAFANA_ENABLED", "false").lower() == "true"
        )
        ...
        self.error_threshold_rate = float(
            os.getenv("STUDIO_ERROR_THRESHOLD", self.error_threshold_rate)
        )
        self.response_time_threshold_ms = int(
            os.getenv("STUDIO_RESPONSE_TIME_THRESHOLD", self.response_time_threshold_ms)
        )
...
class SDKConfig:
    ...
    def __post_init__(self):
        ...
        self.use_database_nodes = (
            os.getenv("STUDIO_SDK_DATABASE_NODES", "true").lower() == "true"
        )
        self.use_security_nodes = (
            os.getenv("STUDIO_SDK_SECURITY_NODES", "true").lower() == "true"
        )
        self.use_ai_nodes = os.getenv("STUDIO_SDK_AI_NODES", "true").lower() == "true"
        self.use_workflow_builder = (
            os.getenv("STUDIO_SDK_WORKFLOW_BUILDER", "true").lower() == "true"
        )
        self.use_async_runtime = (
            os.getenv("STUDIO_SDK_ASYNC_RUNTIME", "true").lower() == "true"
        )
        self.use_business_templates = (
            os.getenv("STUDIO_SDK_BUSINESS_TEMPLATES", "true").lower() == "true"
        )

        # Node configuration
        self.node_discovery_enabled = (
            os.getenv("STUDIO_SDK_NODE_DISCOVERY", "true").lower() == "true"
        )
        self.dynamic_node_loading = (
            os.getenv("STUDIO_SDK_DYNAMIC_LOADING", "true").lower() == "true"
        )
        ...
        if custom_paths_env:
            self.custom_node_paths = [
                path.strip() for path in custom_paths_env.split(",")
            ]

        # Workflow features
        self.workflow_validation_enabled = (
            os.getenv("STUDIO_SDK_WORKFLOW_VALIDATION", "true").lower() == "true"
        )
        self.execution_tracking_enabled = (
            os.getenv("STUDIO_SDK_EXECUTION_TRACKING", "true").lower() == "true"
        )
        self.performance_optimization_enabled = (
            os.getenv("STUDIO_SDK_PERFORMANCE_OPT", "true").lower() == "true"
        )

        # SDK caching
        self.sdk_cache_enabled = (
            os.getenv("STUDIO_SDK_CACHE_ENABLED", "true").lower() == "true"
        )
        self.sdk_cache_ttl_seconds = int(
            os.getenv("STUDIO_SDK_CACHE_TTL", self.sdk_cache_ttl_seconds)
        )
...
class StudioConfig:
    ...
    def __post_init__(self):
        ...
        methods_env = os.getenv(
            "STUDIO_CORS_METHODS", ",".join(self.cors_allow_methods)
        )
        self.cors_allow_methods = [method.strip() for method in methods_env.split(",")]

        headers_env = os.getenv(
            "STUDIO_CORS_HEADERS", ",".join(self.cors_allow_headers)
        )
        self.cors_allow_headers = [header.strip() for header in headers_env.split(",")]

        self.cors_allow_credentials = (
            os.getenv("STUDIO_CORS_CREDENTIALS", "true").lower() == "true"
        )

        # Multi-tenant configuration
        self.default_tenant_id = os.getenv(
            "STUDIO_DEFAULT_TENANT", self.default_tenant_id
        )
        self.multi_tenant_enabled = (
            os.getenv("STUDIO_MULTI_TENANT", "true").lower() == "true"
        )
        self.tenant_isolation_enabled = (
            os.getenv("STUDIO_TENANT_ISOLATION", "true").lower() == "true"
        )

        # Feature flags
        self.enable_auth = os.getenv("STUDIO_ENABLE_AUTH", "true").lower() == "true"
        self.enable_api_docs = (
            os.getenv("STUDIO_ENABLE_API_DOCS", "true").lower() == "true"
        )
        self.enable_admin_interface = (
            os.getenv("STUDIO_ENABLE_ADMIN", "true").lower() == "true"
        )
        self.enable_webhooks = (
            os.getenv("STUDIO_ENABLE_WEBHOOKS", "true").lower() == "true"
        )
        ...
        self.enable_collaboration = (
            os.getenv("STUDIO_ENABLE_COLLABORATION", "true").lower() == "true"
        )
        ...
        return {
            ...
            "handlers": {
                "console": {
                    ...
                    "formatter": (
                        "json" if self.monitoring.log_format == "json" else "default"
                    ),
                    "level": self.monitoring.log_level.value,
                }
            },
            "root": {"level": self.monitoring.log_level.value, "handlers": ["console"]},
        }
...
try:
    ...
    logger.info(
        f"Studio configuration loaded successfully (Environment: {config.environment.value})"
    )
