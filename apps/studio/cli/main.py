...
class StudioGatewayClient:
    ...
    async def get_workflows(self, tenant: str = "default", status: str = None, owner: str = None):
    ...
    async def create_workflow(self, name: str, description: str = "", tenant: str = "default"):
    ...
    async def delete_workflow(self, workflow_id: str, tenant: str = "default"):
        ...
        response = await self.client.delete(f"/api/workflows/{workflow_id}?tenant_id={tenant}")
        return response.status_code == 200
    
    async def get_executions(self, workflow_id: str = None, tenant: str = "default", 
    ...
    async def cancel_execution(self, execution_id: str, tenant: str = "default"):
        ...
        response = await self.client.post(f"/api/executions/{execution_id}/cancel?tenant_id={tenant}")
        return response.status_code == 200
    ...
    async def cleanup_executions(self, days: int = 30, tenant: str = "default"):
        ...
        response = await self.client.post(f"/api/cleanup?days={days}&tenant_id={tenant}")
        response.raise_for_status()
        return response.json()
...
def list_executions(workflow_id, tenant, status, limit):
    ...
    async def list_exec():
        try:
            client = StudioGatewayClient()
            executions_data = await client.get_executions(workflow_id, tenant, status, limit)
            await client.close()
...
def cleanup(days, tenant):
    ...
    async def cleanup_data():
        try:
            ...
            click.echo(f"✅ Cleaned up {deleted} old executions (older than {days} days)")
