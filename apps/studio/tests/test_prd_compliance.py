...
from apps.studio.api.routes import _generate_python_code, add_studio_routes
...
class TestPRDCompliance:
    ...
    def mock_gateway(self):
        ...
        gateway.agent_ui.get_session = AsyncMock(
            return_value=Mock(
                user_id="test-user",
                workflows={
                    "test-workflow": Mock(
        ...
        gateway.agent_ui.create_dynamic_workflow = AsyncMock(
            return_value="new-workflow-id"
        )
        ...
        gateway.ai_chat.process_message = AsyncMock(
            return_value=Mock(content="AI response")
        )

        # Mock workflow generator
        gateway.workflow_generator = AsyncMock()
        gateway.workflow_generator.generate_from_prompt = AsyncMock(
            return_value={
        ...
        gateway.workflow_exporter.to_yaml = Mock(
            return_value="workflow:\n  name: Test\n  nodes: []"
        )

        # Mock node registry
        gateway.node_registry = Mock()
        gateway.node_registry.get_all_nodes = Mock(
            return_value={
        ...
        gateway.schema_registry.get_node_schema = Mock(
            return_value={
    ...
    def test_chat_endpoint_exists(self, client):
        ...
        response = client.post(
            "/api/chat", json={"message": "Create a workflow that reads CSV data"}
        )
    ...
    def test_export_endpoint_exists(self, client):
        ...
        response = client.post(
            "/api/workflows/test-workflow/export?session_id=test-session",
            json={"format": "python"},
        )
    ...
    def test_chat_creates_workflow(self, client, mock_gateway):
        ...
        response = client.post(
            "/api/chat",
            json={"message": "Create a workflow to process CSV data with AI"},
        )
    ...
    def test_python_code_generation(self):
        ...
        workflow.add_node(
            "processor",
            PythonCodeNode(name="processor", code='result = {"count": len(data)}'),
        )
