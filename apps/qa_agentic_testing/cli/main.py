...
from ..core.services import ProjectService, TestRunService, AnalyticsService
from ..core.database import setup_database
from ..core.models import TestType, AgentType, InterfaceType, Priority
...
@click.option('--interfaces', multiple=True, type=click.Choice(['cli', 'web', 'api', 'mobile']),
              help='Interface types to test')
@click.option('--test-types', multiple=True, type=click.Choice(['functional', 'security', 'performance', 'usability', 'integration']),
              help='Test types to include')
@click.option('--output', '-o', type=click.Path(), help='Output directory for reports (defaults to target app\'s qa_results folder)')
@click.option('--format', 'report_format', type=click.Choice(['html', 'json', 'both']), default='html',
              help='Report format')
@click.option('--industry', type=click.Choice(['financial_services', 'healthcare', 'manufacturing', 'retail_ecommerce']),
              help='Load industry-specific personas')
@click.option('--personas', type=click.Path(exists=True), help='Custom personas JSON file')
@click.option('--async', 'use_async', is_flag=True, help='Use async performance mode (3-5x faster)')
@click.option('--discover-personas', is_flag=True, help='Auto-generate personas from app analysis')
@click.option('--model-preset', type=click.Choice(['development', 'balanced', 'enterprise']), 
              default='balanced', help='Model selection preset (development=ollama, balanced=mixed, enterprise=premium)')
@click.option('--security-model', help='Specific model for security analysis (e.g., openai/gpt-4)')
@click.option('--functional-model', help='Specific model for functional testing (e.g., ollama/llama3.1:8b)')
@click.option('--performance-model', help='Specific model for performance analysis (e.g., anthropic/claude-3-sonnet)')
def test(app_path: str, name: Optional[str], description: str, interfaces: tuple, 
         test_types: tuple, output: Optional[str], report_format: str, industry: Optional[str],
         personas: Optional[str], use_async: bool, discover_personas: bool, model_preset: str,
         security_model: Optional[str], functional_model: Optional[str], performance_model: Optional[str]):
    ...
    async def run_test():
        ...
        project_interfaces = [InterfaceType(i) for i in interfaces] if interfaces else [InterfaceType.CLI, InterfaceType.WEB, InterfaceType.API]
        project_test_types = [TestType(t) for t in test_types] if test_types else [TestType.FUNCTIONAL, TestType.SECURITY]
        ...
        try:
            ...
            click.echo(f"📋 Discovered {len(project.discovered_permissions)} permissions")
            ...
            if final_run.status.value == "completed":
                ...
                click.echo(f"📊 Results: {final_run.passed_scenarios}/{final_run.total_scenarios} scenarios passed")
                ...
                if output or report_format:
                    ...
                    if report_format in ['html', 'both']:
                        html_report = await report_service.generate_run_report(run.run_id, 'html')
                        if output:
                            import shutil
                    ...
                    if report_format in ['json', 'both']:
                        json_report = await report_service.generate_run_report(run.run_id, 'json')
                        if output:
                            import shutil
    ...
    async def run_list():
        ...
        if output_format == 'json':
            ...
            for run_item in runs:
                status_icon = "✅" if run_item.status.value == "completed" else "❌" if run_item.status.value == "failed" else "⏳"
                ...
                click.echo(f"   Created: {run_item.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
                click.echo("-" * 100)
...
def run_status(run_id: str):
    ...
    async def get_status():
        ...
        click.echo(f"Status: {run_item.status.value}")
        click.echo(f"Progress: {run_item.passed_scenarios + run_item.failed_scenarios}/{run_item.total_scenarios} scenarios")
        click.echo(f"Success Rate: {run_item.success_rate:.1f}%")
        ...
        if run_item.completed_at:
            click.echo(f"Completed: {run_item.completed_at.strftime('%Y-%m-%d %H:%M:%S')}")
...
def analytics_summary(days: int):
    ...
    async def show_summary():
        ...
        if analytics['projects']:
            click.echo("\n🏆 Top Projects:")
            sorted_projects = sorted(analytics['projects'], key=lambda p: p['success_rate'], reverse=True)
            for project in sorted_projects[:5]:
                click.echo(f"  • {project['name']}: {project['success_rate']:.1f}% ({project['total_runs']} runs)")
...
@models.command('list')
@click.option('--preset', type=click.Choice(['development', 'balanced', 'enterprise']), 
...
def list_models(preset: Optional[str]):
    ...
    for preset_name in presets_to_show:
        ...
        for agent_type in AgentType:
            ...
            click.echo(f"{agent_type.value.ljust(20)}: {config.provider}/{config.model}")
...
@click.option('--security-focused', is_flag=True, help='Prioritize security analysis accuracy')
@click.option('--performance-focused', is_flag=True, help='Prioritize performance analysis')
...
def test_model(provider: str, model: str, prompt: str):
    ...
    try:
        ...
        click.echo(f"✅ Model configuration appears valid")
...
def init():
    ...
    async def initialize():
        ...
        click.echo("  • Run 'qa-test /path/to/your/app --async' for 3-5x faster testing")
