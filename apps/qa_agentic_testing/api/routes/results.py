...
async def list_results_for_run(
    run_id: str,
    service: TestResultService = Depends(get_result_service)
):
    ...
    try:
        ...
        for test_type, data in by_type.items():
            if data["total"] > 0:
                ...
                data["average_confidence"] = sum(r["confidence_score"] for r in data["results"]) / data["total"]
...
async def get_performance_metrics(
    ...
    try:
        ...
        memory_usage = [r.memory_usage_mb for r in results if r.memory_usage_mb is not None]
        cpu_usage = [r.cpu_usage_percent for r in results if r.cpu_usage_percent is not None]
