        ...
        if project_id:
            ...
            for project in projects:
                if project["completed_runs"] > 0:
                    project_analytics = await service.get_project_analytics(project["project_id"], days)
                    performance_data.append({
                        "project_id": project["project_id"],
                        "project_name": project["name"],
                        "average_duration": project_analytics.get("average_duration_seconds", 0),
                        "performance_trend": project_analytics.get("performance_trend", [])
                    })
...
async def get_analytics_summary(
    ...
    try:
        ...
        # Calculate trends
        success_rate_change_7d = last_7_days["average_success_rate"] - last_30_days["average_success_rate"]
        ...
        return {
            ...
            "top_projects": sorted(
                last_30_days["projects"], 
                key=lambda p: p["success_rate"], 
                reverse=True
            )[:5],
            "active_projects": [
                p for p in last_30_days["projects"] 
                if p["total_runs"] > 0
            ]
        }
...
async def get_system_health(
    ...
    try:
        ...
        recent_successful_runs = len([r for r in recent_runs if r.status.value == "completed"])
        ...
        return {
            ...
            "metrics": {
                ...
                "failure_rate": recent_failed_runs / len(recent_runs) if recent_runs else 0
            },
            "recommendations": _get_health_recommendations(health_score, recent_runs, projects)
        }
...
def _get_health_recommendations(health_score: float, recent_runs: list, projects: list) -> list:
    """Generate health recommendations based on metrics."""
    recommendations = []
    
    if health_score < 80:
        recommendations.append("System health is below optimal. Consider reviewing failed test runs.")
    
    if not recent_runs:
        recommendations.append("No recent test activity. Consider running tests for active projects.")
    
    inactive_projects = len([p for p in projects if not p.is_active])
    if inactive_projects > len(projects) * 0.5:
        recommendations.append(f"Many projects ({inactive_projects}) are inactive. Consider archiving or reactivating them.")
    
    failed_runs = [r for r in recent_runs if r.status.value == "failed"]
    if len(failed_runs) > len(recent_runs) * 0.3:
        recommendations.append("High failure rate detected. Review test configurations and application stability.")
