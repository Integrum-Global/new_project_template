...
async def test_working_components():
    ...
    try:
        ...
        # Create test objects
        project = models.create_test_project("Test Project", "/tmp/test", "Test description")
        run = models.create_test_run(project.project_id, "Test Run", "Test run description")
    ...
    try:
        ...
        project = models.create_test_project("DB Test Project", "/tmp/db_test", "DB test")
    ...
    try:
        ...
        generated_personas = persona_manager.generate_personas_from_permissions(mock_permissions)
    ...
    try:
        ...
        test_personas = list(personas_dict.values())[:3] if len(personas_dict) >= 3 else list(personas_dict.values())
        ...
        # Generate scenarios
        scenarios = scenario_generator.generate_scenarios_for_personas(test_personas, mock_analysis)
