...
class PersonaManager:
    ...
    def __init__(self, templates_dir: Optional[Path] = None):
        ...
        self.templates_dir = templates_dir or Path(__file__).parent.parent / "templates" / "personas"
    ...
    def _load_built_in_personas(self):
        ...
        self.add_persona(Persona(
            ...
            goals=["Maintain system security", "Manage all users", "Monitor system activity", "Ensure compliance"],
            behavior_style="Systematic, security-conscious, detail-oriented, thorough",
            typical_actions=["Create users", "Assign roles", "Review audit logs", "Configure settings", "Monitor performance"],
        ...
        self.add_persona(Persona(
            ...
            goals=["Monitor security events", "Investigate anomalies", "Generate compliance reports", "Ensure security policies"],
            behavior_style="Vigilant, investigative, compliance-focused, thorough",
            typical_actions=["Review audit logs", "Monitor login patterns", "Investigate security events", "Generate reports"],
        ...
        self.add_persona(Persona(
            ...
            goals=["Manage team access", "Review team activity", "Request access changes", "Monitor team performance"],
            behavior_style="Goal-oriented, efficiency-focused, team-centric, results-driven",
            typical_actions=["View team members", "Update team info", "Review team reports", "Request role changes"],
        ...
        self.add_persona(Persona(
            ...
            goals=["Generate business reports", "Analyze trends", "Export data", "Create dashboards"],
            behavior_style="Data-driven, analytical, detail-oriented, methodical",
            typical_actions=["Run reports", "Export data", "Analyze trends", "Create visualizations"],
        ...
        self.add_persona(Persona(
            ...
            goals=["Access needed resources", "Update own information", "Complete daily tasks"],
            behavior_style="Task-focused, efficiency-seeking, cautious with new features",
            typical_actions=["View own profile", "Update personal info", "Access assigned resources"],
        ...
        self.add_persona(Persona(
            ...
            goals=["Learn the system", "Complete onboarding", "Get necessary access"],
            behavior_style="Curious, cautious, question-asking, learning-oriented",
            typical_actions=["Explore interface", "Follow tutorials", "Ask for help", "Try basic features"],
        ...
        self.add_persona(Persona(
            ...
            goals=["Use advanced features", "Integrate with other tools", "Maximize productivity"],
            behavior_style="Experienced, efficiency-focused, feature-seeking, technical",
            typical_actions=["Use advanced features", "Configure integrations", "Automate workflows"],
            expected_success_rate=95.0,
            test_focus=["advanced_features", "performance", "integration", "automation"]
        ))
        ...
        if self.templates_dir.exists():
            for template_file in self.templates_dir.glob("*.json"):
                try:
                    ...
                    print(f"Warning: Could not load persona template {template_file}: {e}")
    ...
    def generate_custom_persona_interactive(self) -> Persona:
        ...
        # Behavior style
        behavior_style = input("\nBehavior style (e.g., 'detail-oriented, analytical'): ").strip()
    ...
    def generate_personas_from_permissions(self, permission_patterns: List[str]) -> List[Persona]:
    ...
    def _create_persona_from_permissions(self, key: str, name: str, role: str, permissions: List[str]) -> Persona:
    ...
    def get_testing_matrix(self, selected_personas: Optional[List[str]] = None) -> List[Persona]:
        """Get a curated list of personas for comprehensive testing."""
        if selected_personas:
            return [self.personas[key] for key in selected_personas if key in self.personas]
        
        # Default testing matrix covers major user types
        default_personas = ["system_admin", "manager", "regular_user", "security_officer"]
        available_personas = [self.personas[key] for key in default_personas if key in self.personas]
        
        # Add any additional personas if we don't have enough coverage
        if len(available_personas) < 3:
            additional = [p for p in self.personas.values() if p.key not in default_personas]
    ...
    async def generate_personas_from_permissions_async(self, permissions: List[str]) -> List[Persona]:
