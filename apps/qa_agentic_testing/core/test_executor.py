...
from .scenario_generator import ScenarioGenerator, TestScenario, ScenarioType, InterfaceType
...
class AutonomousQATester:
    ...
    async def discover_app(self, app_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        self.logger.info(f"Discovery completed. Found {len(self.app_analysis['interfaces'])} interfaces, "
    ...
    async def _discover_interfaces(self):
        ...
        if InterfaceType.CLI not in interfaces:
            for py_file in self.app_path.rglob("*.py"):
                try:
                    ...
                    cli_patterns = ["argparse", "click", "typer", "sys.argv", "ArgumentParser", "@click.", "@typer."]
        ...
        api_content_indicators = ["fastapi", "flask", "django", "from fastapi", "from flask", "from django"]
        
        # Check for API files by name
        for indicator in api_file_indicators:
            if any(f.name.lower().find(indicator) != -1 for f in self.app_path.rglob("*.py")):
        ...
        if InterfaceType.API not in interfaces:
            for py_file in self.app_path.rglob("*.py"):
                try:
                    async with aiofiles.open(py_file, 'r', encoding='utf-8') as f:
                        content = await f.read()
                    if any(pattern in content.lower() for pattern in api_content_indicators):
                        interfaces.append(InterfaceType.API)
                        break
                except Exception:
                    continue
        ...
        interface_strings = [iface.value if hasattr(iface, 'value') else str(iface) for iface in interfaces]
        ...
        for py_file in self.app_analysis.get("operations", []):
            ...
            try:
                ...
                for line in content.split('\n'):
                    if any(keyword in line.lower() for keyword in ['permission', 'auth', 'role']):
                        # Extract potential permissions
                        if ':' in line:
                            parts = line.split(':')
                            for part in parts:
                                part = part.strip().strip('\'"')
                                if len(part) > 2 and len(part) < 50 and part.replace('_', '').replace(':', '').isalnum():
                                    permissions.add(part)
                        
            except Exception as e:
                self.logger.warning(f"Could not extract permissions from {file_path}: {e}")
        
        # Add default permissions if none found
        if not permissions:
            permissions.update([
                "user:read", "user:create", "user:update", "user:delete",
                "admin:all", "manager:team"
            ])
    ...
    async def _analyze_existing_tests(self):
        ...
        self.app_analysis["existing_test_patterns"] = test_patterns[:10]  # Limit to first 10
    ...
    def _infer_operation_type(self, func_name: str) -> str:
        ...
        if any(keyword in name_lower for keyword in ['create', 'add', 'new', 'insert']):
            return "create"
        elif any(keyword in name_lower for keyword in ['read', 'get', 'fetch', 'list', 'show']):
            return "read"
    ...
    async def _infer_interface_from_file(self, file_path: Path) -> str:
        ...
        if any(part in relative_path for part in ['cli', 'console', 'command']):
            ...
            return "api"
        elif any(part in relative_path for part in ['web', 'ui', 'frontend', 'templates']):
            return "web"
        ...
        try:
            ...
            # API patterns  
            api_patterns = ["@app.route", "@router.", "FastAPI", "Flask", "@api.", "APIRouter"]
            if any(pattern in content for pattern in api_patterns):
                return "api"
            
            # Web patterns
            web_patterns = ["render_template", "HttpResponse", "redirect", "request.GET", "request.POST"]
    ...
    def generate_personas(self, custom_personas: Optional[List[Dict[str, Any]]] = None) -> List[Persona]:
        ...
        if len(testing_personas) < 4 and self.app_analysis.get("permissions"):
            ...
            self.logger.info(f"Generated {len(generated)} additional personas from permissions")
            # Refresh testing matrix to include new personas
            testing_personas = self.persona_manager.get_testing_matrix()
        
        self.logger.info(f"Selected {len(testing_personas)} personas for testing: {[p.name for p in testing_personas]}")
    ...
    def _generate_scenarios_sync(
        ...
        self.logger.info(f"Generating scenarios for {len(personas)} personas and {len(scenario_types)} scenario types")
        
        # Use discovered app analysis for scenario generation
        app_analysis = self.app_analysis if self.app_analysis else self._get_default_app_analysis()
        
        self.generated_scenarios = self.scenario_generator.generate_scenarios_for_personas(
    ...
    async def _generate_scenarios_async(
        ...
        self.logger.info(f"Generating scenarios asynchronously for {len(personas)} personas and {len(scenario_types)} scenario types")
        
        # Use discovered app analysis for scenario generation
        app_analysis = self.app_analysis if self.app_analysis else self._get_default_app_analysis()
        
        # Use async scenario generation for better performance
        self.generated_scenarios = await self.scenario_generator.generate_scenarios_for_personas_async(
            personas, app_analysis, scenario_types
        )
        
        self.logger.info(f"Generated {len(self.generated_scenarios)} test scenarios asynchronously")
        return self.generated_scenarios
    ...
    def _get_default_app_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "operations": [
                {"name": "list_items", "type": "read", "permissions_required": ["item:read"]},
                {"name": "create_item", "type": "create", "permissions_required": ["item:create"]},
                {"name": "update_item", "type": "update", "permissions_required": ["item:update"]},
                {"name": "delete_item", "type": "delete", "permissions_required": ["item:delete"]}
            ],
    ...
    async def execute_tests(
        ...
        if not scenarios:
            raise ValueError("No scenarios available for execution. Run generate_scenarios() first.")
        
        max_concurrent = max_concurrent or self.config["testing"]["max_concurrent_tests"]
        
        self.logger.info(f"Executing {len(scenarios)} test scenarios with max {max_concurrent} concurrent")
        ...
        self.logger.info(f"Test execution completed. Success rate: {self.execution_summary.success_rate:.1f}%")
        return self.execution_summary
    
    async def _execute_single_test(self, test_id: str, scenario: TestScenario, persona: Persona) -> TestResult:
        ...
        try:
            ...
            for step in scenario.steps:
                ...
                step_results.append({
                    ...
                    "expected": step.step_id not in expectations["expected_failures"]
                })
                
                execution_log.append(f"Step {step.step_id}: {'PASS' if step_success else 'FAIL'} ({step_duration:.3f}s)")
            
            # Perform LLM analysis
            llm_analysis = None
            if self.config["llm"]["use_consensus"]:
                try:
                    analysis_prompt = self._create_analysis_prompt(scenario, persona, step_results)
                    persona_context = {
                        "name": persona.name,
                        "role": persona.role,
            ...
            performance_metrics = {
                ...
                "average_step_duration": sum(r["duration"] for r in step_results) / len(step_results),
                "success_rate": sum(1 for r in step_results if r["success"]) / len(step_results) * 100
            }
            
            # Calculate confidence score
            confidence_score = self._calculate_test_confidence(validation_result, llm_analysis, step_results)
    ...
    def _create_analysis_prompt(self, scenario: TestScenario, persona: Persona, step_results: List[Dict]) -> str:
...
"""
    ...
    def _validate_test_results(
        ...
        success_rate = validation_details["passed_steps"] / validation_details["total_steps"] * 100
        ...
        elif success_rate < expected_success_rate - 20:  # Significantly worse than expected
            validation_result = ValidationResult.WARNING
        else:
    ...
    def _calculate_test_confidence(
        ...
        if step_success_rate > 0.8 or step_success_rate < 0.2:  # Very high or very low success rate
    ...
    def _generate_execution_summary(self) -> TestExecutionSummary:
        ...
        # Duration metrics
        durations = [r.duration_seconds for r in self.test_results if r.duration_seconds > 0]
        ...
        performance_summary = {
            ...
            "tests_per_minute": len(self.test_results) / (total_duration / 60) if total_duration > 0 else 0
        }
        ...
        for result in self.test_results:
            if result.persona_key not in persona_results:
                persona_results[result.persona_key] = {"pass": 0, "fail": 0, "warning": 0, "total": 0}
        ...
        for result in self.test_results:
            if result.scenario_id not in scenario_results:
                scenario_results[result.scenario_id] = {"pass": 0, "fail": 0, "warning": 0, "total": 0}
    ...
    async def generate_report(
        ...
        report_data = {
            "test_info": {
                ...
                "duration_estimated_minutes": int((time.time() - self.config.get("start_time", time.time())) / 60) + 1,
                "app_name": self.app_path.name if self.app_path else "Unknown Application",
            ...
            "personas": {key: persona.to_dict() for key, persona in self.persona_manager.personas.items()},
            "llm_analysis_history": [analysis.to_dict() for analysis in self.agent_coordinator.analysis_history]
        }
        ...
        if format == "html" or format == "both":
            ...
            await self.report_generator.generate_comprehensive_report_async(adapted_data, html_file)
        ...
        if format == "json" or format == "both":
            ...
            await self.report_generator.generate_json_report_async(report_data, json_file)
    ...
    async def quick_test(self, app_path: Path, output_dir: Optional[Path] = None) -> TestExecutionSummary:
