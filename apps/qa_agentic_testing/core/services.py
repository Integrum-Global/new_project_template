...
class ProjectService:
    ...
    async def create_project(self, name: str, app_path: str, 
                           description: str = "", **kwargs) -> TestProject:
    ...
    async def list_projects(self, tenant_id: str = "default", 
                          is_active: bool = True) -> List[TestProject]:
        """List all projects."""
        return await self.db.list_projects(tenant_id, is_active)
    
    async def update_project(self, project_id: str, 
                           updates: Dict[str, Any]) -> Optional[TestProject]:
        """Update a project."""
    ...
    async def discover_project(self, project_id: str) -> Optional[TestProject]:
        ...
        await self._discover_project(project)
        return await self.db.update_project(project_id, {
...
class TestRunService:
    ...
    async def create_run(self, project_id: str, name: str, 
                        description: str = "", **kwargs) -> TestRun:
    ...
    async def _execute_run(self, run_id: str):
        ...
        try:
            ...
            await self.db.update_run(run_id, {
                ...
                "completed_at": datetime.now(timezone.utc).isoformat(),
                "duration_seconds": (datetime.now(timezone.utc) - run.started_at).total_seconds() if run.started_at else 0,
    ...
    async def _update_project_stats(self, project_id: str):
        ...
        if completed_runs:
            avg_success_rate = sum(r.success_rate for r in completed_runs) / len(completed_runs)
...
class TestResultService:
    ...
    async def create_result(self, run_id: str, scenario_id: str, 
                          scenario_name: str, test_type: TestType,
                          persona: str, interface_type: InterfaceType,
                          **kwargs) -> TestResult:
        """Create a new test result."""
        result = create_test_result(
            run_id, scenario_id, scenario_name, 
            test_type, persona, interface_type
        )
...
class AnalyticsService:
    ...
    async def get_project_analytics(self, project_id: str, 
        ...
        avg_success_rate = sum(r.success_rate for r in completed_runs) / len(completed_runs) if completed_runs else 0
        avg_duration = sum(r.duration_seconds for r in completed_runs if r.duration_seconds) / len(completed_runs) if completed_runs else 0
        avg_confidence = sum(r.confidence_score for r in completed_runs) / len(completed_runs) if completed_runs else 0
    ...
    async def get_global_analytics(self, tenant_id: str = "default", 
                                 days: int = 30) -> Dict[str, Any]:
        ...
        avg_success_rate = avg_success_rate / len([p for p in project_analytics if p["completed_runs"] > 0]) if project_analytics else 0
...
class ReportService:
    ...
    async def generate_run_report(self, run_id: str, 
