...
class TestScenario:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TestScenario":
        ...
        data["interface_types"] = [InterfaceType(iface) for iface in data["interface_types"]]
...
class ScenarioGenerator:
    ...
    def __init__(self, templates_dir: Optional[Path] = None):
        self.templates_dir = templates_dir or Path(__file__).parent.parent / "templates" / "scenarios"
        ...
        if self.templates_dir.exists():
            for template_file in self.templates_dir.glob("*.json"):
                try:
                    ...
                    print(f"Warning: Could not load scenario template {template_file}: {e}")
    ...
    def analyze_application(self, app_structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis["performance_targets"] = app_structure.get("performance_targets", {
    ...
    def generate_scenarios_for_personas(
        ...
        for scenario_type in scenario_types:
            for persona in personas:
                scenario = self._generate_scenario_for_persona(persona, scenario_type, app_analysis)
                if scenario:
    ...
    def _generate_functional_scenario(self, persona: Persona, app_analysis: Dict[str, Any]) -> TestScenario:
        ...
        for interface in app_analysis["interfaces"]:
            ...
            if "authentication" in app_analysis["security_features"]:
                steps.append(TestStep(
                    ...
                    validation_criteria=["Authentication successful", "Session established"],
                    interface=interface,
            ...
            for operation in app_analysis["operations"]:
                if self._persona_can_attempt_operation(persona, operation):
                    expected_success = self._should_operation_succeed(persona, operation)
                    
                    steps.append(TestStep(
                        step_id=f"step_{step_counter}",
                        description=f"Attempt {operation['name']} operation",
                        action=operation["name"],
                        expected_result="Success" if expected_success else "Permission denied",
                        validation_criteria=self._get_operation_validation_criteria(operation, expected_success),
                        interface=interface,
                        parameters={"operation": operation["name"], "persona_permissions": persona.permissions}
                    ))
                    step_counter += 1
    ...
    def _generate_security_scenario(self, persona: Persona, app_analysis: Dict[str, Any]) -> TestScenario:
        ...
        steps.append(TestStep(
            ...
            interface=InterfaceType.API if InterfaceType.API in app_analysis.get("interfaces", []) else (app_analysis.get("interfaces", [InterfaceType.CLI])[0] if app_analysis.get("interfaces") else InterfaceType.CLI),
        ...
        if any("create" in op["name"].lower() for op in app_analysis["operations"]):
            steps.append(TestStep(
                ...
                interface=InterfaceType.API if InterfaceType.API in app_analysis.get("interfaces", []) else (app_analysis.get("interfaces", [InterfaceType.CLI])[0] if app_analysis.get("interfaces") else InterfaceType.CLI),
                parameters={"test_payloads": ["'; DROP TABLE users; --", "<script>alert('xss')</script>"]}
            ))
            step_counter += 1
        ...
        return TestScenario(
            ...
            tags=["security", "permissions", "validation", persona.role.lower().replace(" ", "_")]
        )
    
    def _generate_performance_scenario(self, persona: Persona, app_analysis: Dict[str, Any]) -> TestScenario:
        ...
        steps.append(TestStep(
            ...
            interface=InterfaceType.API if InterfaceType.API in app_analysis.get("interfaces", []) else (app_analysis.get("interfaces", [InterfaceType.CLI])[0] if app_analysis.get("interfaces") else InterfaceType.CLI),
            parameters={"iterations": 10, "target_ms": performance_targets.get('response_time_ms', 200)}
        ))
        step_counter += 1
        ...
        if len(persona.permissions) > 2:  # Avoid load testing with restricted users
            steps.append(TestStep(
                ...
                interface=InterfaceType.API if InterfaceType.API in app_analysis.get("interfaces", []) else (app_analysis.get("interfaces", [InterfaceType.CLI])[0] if app_analysis.get("interfaces") else InterfaceType.CLI),
                parameters={
                    "concurrent_users": performance_targets.get('concurrent_users', 10),
                    "duration_seconds": 30
                }
            ))
            step_counter += 1
        ...
        return TestScenario(
            ...
            tags=["performance", "load_testing", "response_time", persona.role.lower().replace(" ", "_")]
        )
    
    def _generate_usability_scenario(self, persona: Persona, app_analysis: Dict[str, Any]) -> TestScenario:
        ...
        steps.append(TestStep(
            ...
            interface=app_analysis.get("interfaces", [InterfaceType.CLI])[0] if app_analysis.get("interfaces") else InterfaceType.CLI,
            parameters={"persona_level": "beginner" if "new" in persona.key else "experienced"}
        ))
        step_counter += 1
        ...
        return TestScenario(
            ...
            tags=["usability", "user_experience", "navigation", persona.role.lower().replace(" ", "_")]
        )
    
    def _generate_integration_scenario(self, persona: Persona, app_analysis: Dict[str, Any]) -> TestScenario:
        ...
        if len(app_analysis["interfaces"]) > 1:
            steps.append(TestStep(
                ...
                parameters={"interfaces_to_test": [iface.value for iface in app_analysis["interfaces"]]}
            ))
            step_counter += 1
        ...
        return TestScenario(
            ...
            tags=["integration", "cross_interface", persona.role.lower().replace(" ", "_")]
        )
    
    def _persona_can_attempt_operation(self, persona: Persona, operation: Dict[str, Any]) -> bool:
    ...
    def _should_operation_succeed(self, persona: Persona, operation: Dict[str, Any]) -> bool:
    ...
    def _get_operation_validation_criteria(self, operation: Dict[str, Any], should_succeed: bool) -> List[str]:
    ...
    def generate_regression_scenarios(self, baseline_results: Dict[str, Any]) -> List[TestScenario]:
        ...
        for scenario_id, results in baseline_results.items():
            ...
            regression_scenario = TestScenario(
                ...
                interface_types=[InterfaceType(iface) for iface in results.get("interface_types", ["api"])],
                priority="high",
                estimated_duration_minutes=results.get("estimated_duration_minutes", 10),
    ...
    def get_scenario_matrix(self, personas: List[Persona], scenario_types: List[ScenarioType]) -> List[TestScenario]:
    ...
    def export_scenarios(self, file_path: Path):
        ...
        scenarios_data = [scenario.to_dict() for scenario in self.generated_scenarios.values()]
    ...
    def get_scenarios_by_type(self, scenario_type: ScenarioType) -> List[TestScenario]:
        ...
        return [s for s in self.generated_scenarios.values() if s.scenario_type == scenario_type]
    
    def get_estimated_total_duration(self) -> int:
        """Get total estimated duration for all scenarios in minutes."""
        return sum(scenario.estimated_duration_minutes for scenario in self.generated_scenarios.values())
    ...
    async def generate_scenarios_for_personas_async(
        ...
        self.logger.info(f"Generating scenarios for {len(personas)} personas and {len(scenario_types)} types")
        
        # Create tasks for concurrent scenario generation
        tasks = []
        for persona in personas:
            for scenario_type in scenario_types:
                task = self._generate_scenario_async(persona, scenario_type, app_analysis)
                tasks.append(task)
    ...
    async def get_scenario_matrix_async(
        ...
        return await self.generate_scenarios_for_personas_async(personas, {}, scenario_types)