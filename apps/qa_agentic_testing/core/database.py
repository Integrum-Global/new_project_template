...
class QADatabase:
    ...
    async def _create_tables(self, conn: aiosqlite.Connection):
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_test_runs_project_id ON test_runs (project_id)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_test_runs_status ON test_runs (status)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_test_results_run_id ON test_results (run_id)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_test_results_status ON test_results (status)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_agent_sessions_run_id ON agent_sessions (run_id)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_test_metrics_project_id ON test_metrics (project_id)")
    ...
    async def create_project(self, project: TestProject) -> TestProject:
        ...
        async with self.get_connection() as conn:
            ...
            """, (
                project.project_id, project.name, project.description, project.app_path,
                ...
                project.created_by, project.is_active, project.tenant_id,
                json.dumps(project.tags), json.dumps(project.attributes),
                project.total_test_runs,
                project.last_test_run.isoformat() if project.last_test_run else None,
    ...
    async def list_projects(self, tenant_id: str = "default", is_active: bool = True) -> List[TestProject]:
    ...
    async def update_project(self, project_id: str, updates: Dict[str, Any]) -> Optional[TestProject]:
        """Update a test project."""
        updates["updated_at"] = datetime.now(timezone.utc).isoformat()
        
        # Handle JSON fields
        json_fields = ["interfaces", "test_types", "agent_config", "discovered_permissions",
                      "discovered_endpoints", "discovered_commands", "tags", "attributes"]
    ...
    async def create_run(self, run: TestRun) -> TestRun:
        ...
        async with self.get_connection() as conn:
            ...
            """, (
                run.run_id, run.project_id, run.name, run.description,
                ...
                run.duration_seconds, run.total_scenarios, run.passed_scenarios,
                run.failed_scenarios, run.skipped_scenarios, run.success_rate,
                json.dumps(run.ai_insights), run.confidence_score,
                json.dumps(run.agent_consensus), run.created_at.isoformat(),
                run.created_by, run.tenant_id, run.priority.value,
                run.html_report_path, run.json_report_path, run.error_message,
    ...
    async def list_runs(self, project_id: Optional[str] = None, 
                       tenant_id: str = "default", limit: int = 50) -> List[TestRun]:
        """List test runs."""
    ...
    async def update_run(self, run_id: str, updates: Dict[str, Any]) -> Optional[TestRun]:
        """Update a test run."""
        # Handle JSON fields
        json_fields = ["test_types", "personas_used", "agent_types", "interfaces_tested",
                      "ai_insights", "agent_consensus", "error_details"]
        for field in json_fields:
            if field in updates and updates[field] is not None:
                if isinstance(updates[field], list) and field in ["test_types", "agent_types", "interfaces_tested"]:
                    # Handle enum lists
                    updates[field] = json.dumps(updates[field])
    ...
    async def create_result(self, result: TestResult) -> TestResult:
        ...
        async with self.get_connection() as conn:
            ...
            """, (
                result.result_id, result.run_id, result.scenario_id, result.scenario_name,
                result.test_type.value, result.persona, result.interface_type.value,
                result.status.value, result.started_at.isoformat(),
                result.completed_at.isoformat() if result.completed_at else None,
                result.duration_ms, result.passed, result.expected_result,
                result.actual_result, json.dumps(result.validation_details),
                result.ai_analysis, result.confidence_score,
                json.dumps(result.agent_insights), result.response_time_ms,
                result.memory_usage_mb, result.cpu_usage_percent,
                result.error_message, result.error_traceback, result.tenant_id
            ))
            await conn.commit()
        return result
    ...
    def _row_to_project(self, row) -> TestProject:
        ...
        return TestProject(
            ...
            interfaces=[InterfaceType(i) for i in json.loads(row["interfaces"] or "[]")],
            ...
            last_test_run=datetime.fromisoformat(row["last_test_run"]) if row["last_test_run"] else None,
    ...
    def _row_to_run(self, row) -> TestRun:
        ...
        return TestRun(
            ...
            interfaces_tested=[InterfaceType(i) for i in json.loads(row["interfaces_tested"] or "[]")],
